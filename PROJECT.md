...
**Last consolidated:** 2026-04-21 session 3 (post-polish cutover). **Last session-index update:** 2026-04-28 (eve wrap) (HomeDashboard drift Phase 0 + Initiative C Phase 1A schema contracts + Init C v1 plan capture).
...
- **`client_submission_id` is the queue idempotency key on 9 webform-target tables** (migration 030, 2026-04-28 eve queue). Nullable text + non-partial unique index on each of: `pig_dailys`, `poultry_dailys`, `layer_dailys`, `cattle_dailys`, `sheep_dailys`, `weigh_in_sessions`, `weigh_ins`, `equipment_fuelings`, `fuel_supplies`. Index has NO `WHERE … IS NOT NULL` predicate — PostgREST's `.upsert(onConflict: 'client_submission_id')` cannot express partial-index predicates in the conflict target, so partial indexes silently fail at upsert time. Postgres NULLS DISTINCT keeps multiple legacy null rows valid. Future queue replay relies on `.upsert({onConflict: 'client_submission_id', ignoreDuplicates: true})` for at-least-once safety; if you change the index shape, run a Playwright spec that double-submits the same csid to confirm the second insert is a no-op rather than a 409/duplicate-key error.
...
- **Initiative C — Mobile install + offline/photo-capable webform queue.** Plan-approved 2026-04-28 (eve+). Phase 1A (DB schema contracts) shipped 2026-04-28 (eve queue, commit `9e93e0a`). Migrations 030 + 031 not yet applied to prod — Ronnie applies via Supabase SQL Editor when convenient before Phase 1B starts. See "Initiative C v1 plan capture" below for full scope.
...
- Idempotency: `client_submission_id text` (client-generated UUID/id, no specific package locked-in). Non-partial unique index. PostgREST `.upsert(onConflict: 'client_submission_id', ignoreDuplicates: true)` for at-least-once safety.
...
**Phase plan:**
- ~~**Phase 1A — DB schema contracts.**~~ **DONE** 2026-04-28 (eve queue, commit `9e93e0a`). Migrations 030 (CSID + photos columns) + 031 (daily-photos bucket + policies). Pending Ronnie's prod apply.
- **Phase 1B — adapter foundation + canary.** `idb` install, `src/lib/offlineQueue.js` (IndexedDB wrapper), `src/lib/offlineForms.js` (form_kind registry), `src/lib/photoCompress.js`, `src/lib/clientSubmissionId.js`. PWA `manifest.webmanifest` + install scaffolding (no Service Worker caching yet — Phase 3). Wire FuelSupplyWebform as canary (text-only; adapter records carry empty `photo_refs:[]`). `useOfflineSubmit(form_kind, payload)` hook. ~1.5 sessions estimated.
- **Phase 1C — fan-out across remaining text forms.** Wire WeighInsWebform (alongside existing drafts), AddFeedWebform, PigDailysWebform. Each = one registry entry + hook call (~30 LOC per form). ~1 session.
- **Phase 2 — first photo-capable form.** Wire EquipmentFuelingWebform photo flow against the new `daily-photos` bucket. Switch its existing photo path scheme from `Date.now()-based` to deterministic `<form_kind>/<client_submission_id>/<photo_key>.jpg`. Don't migrate historical photos — leave existing prod paths untouched (the photo→fueling matcher already keys on `(equipment_id, date)`, not path). ~1 session.
- **Phase 3 — daily-report photo capture + Service Worker shell.** UI for photo capture on all 5 daily-report forms. SW caches app shell + webform routes. Cache strategies for `webform_config` reads (cache-first, revalidate hourly). Optional install prompt UX. ~1–2 sessions.

**Test plan (each phase's gates):**
- Vitest: `offlineQueue.test.js`, `offlineForms.test.js`, `photoCompress.test.js`, `clientSubmissionId.test.js`.
- Playwright: `offline_queue_canary.spec.js`, `offline_queue_dedup.spec.js` (Phase 1), `offline_queue_multi_form.spec.js` (Phase 1C), `offline_photo_upload.spec.js`, `offline_photo_dedup.spec.js`, `offline_storage_quota.spec.js` (Phase 2). Network blocking via Playwright route stubs.

**Pre-Phase-1B prerequisites:**
1. Ronnie applies migrations 030 + 031 to prod via Supabase SQL Editor.
2. Verify post-apply: `pig_dailys`, `poultry_dailys`, `layer_dailys`, `cattle_dailys`, `sheep_dailys` each have a `client_submission_id` column + a `photos` jsonb column. The other 4 tables (weigh_in_sessions, weigh_ins, equipment_fuelings, fuel_supplies) only get the CSID column. `daily-photos` bucket exists with `public=false`.

### Next build

- **Initiative C Phase 1B (adapter foundation + FuelSupply canary).** Plan-locked. Awaits prod apply of migrations 030 + 031.
