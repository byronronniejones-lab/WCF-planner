...
**Last consolidated:** 2026-04-21 session 3 (post-polish cutover). **Last session-index update:** 2026-04-28 PM (Playwright initiative wrap — A5 / A6 / A7 / A9 / A8a; A8b queued).
...
### Recommended sequencing (set 2026-04-27, updated 2026-04-28 PM)

Playwright initiative is one spec away from wrap (A8b), then A10 unblocks once Initiative B lands. All test work tracked below has SHIPPED to production unless flagged otherwise.
...
3. **Playwright integration tests** — 8 of 9 planned specs shipped, including smoke; A8b queued.
   - ~~**A2 harness**~~ **DONE** + **PUSHED** (commit `0ad8fc2`). Local-only Playwright runner against isolated Supabase test project. `assertTestDatabase` guard, 29-table truncate whitelist, `exec_sql` RPC, single-bundle bootstrap generator, DEV-only backend sentinel, `--strictPort` to prevent zombie-dev-server contamination.
   - ~~**A4 pig batch math spec**~~ **DONE** + **PUSHED** (commit `3234ff6`). Send-to-Trip happy-path; subAttributions schema, ledger current count, lbs/pig finishers-denominator regression for the 1644 vs 1186 P-26-01A bug.
   - ~~**A5 cattle Send-to-Processor spec**~~ **DONE** + **PUSHED** (commit `e320237`, 2026-04-28 PM). 9 tests: attach, toggle-clear, entry-delete, session-delete (3-cow loop), batch-delete (3-cow loop, drives DeleteModal UI), audit-row fallback, null-from-herd truthy guard, no-prior-herd block, no-manual-bypass negative. Production diff: 1-line empty-state copy patch (`CattleBatchesView.jsx:192` "you can attach cattle from the Herds tab" → "Cattle enter via Send-to-Processor on a finisher weigh-in entry").
   - ~~**A6 sheep Send-to-Processor spec**~~ **DONE** + **PUSHED** (commit `7101484`, 2026-04-28 PM). 10 tests: A5 mirror + 1 unique looser-gate test (rams flock CAN attach; cattle would refuse). Production diff: ~9-line copy alignment across SheepBatchesView (3 places) + SheepHomeView (1) + SheepWeighInsView (1 comment) + SheepSendToProcessorModal (title + comment) — replaces stale "feeders weigh-in entry" wording with "sheep weigh-in entry" matching the §7 any-flock contract. Modal title simplified to "Send N sheep to processor" (sheep is invariant plural).
   - ~~**A7 broiler timeline spec**~~ **DONE** + **PUSHED** (commit `be7df4c`, 2026-04-28 PM). 4 tests: range derivation (broiler-only, layer-included + Retirement-Home-excluded combined), auto-scroll ~12% from left, today-line indicator. Production diff: 3 DOM hooks added to `BroilerTimelineView.jsx` — `data-week-header="1"` + `data-iso={toISO(w)}` on labeled week cells, `data-today-line="1"` on the today indicator. No logic change.
   - ~~**A9 FCR cache spec (Edit Trip + Delete Trip)**~~ **DONE** + **PUSHED** (commit `b089505`, 2026-04-28 PM). Closes the contract A4 deferred. 3 tests: Edit→Close populates fcrCached + preserves non-empty subAttributions; Edit→Close DELETES key (Object.hasOwn=false) when adjFeed≤credits; Delete Trip via real DeleteModal clears the key. Zero production diff.
   - ~~**A8a fuel bill PDF parser end-to-end**~~ **DONE** + **PUSHED** (commit `d9fa8ba`, 2026-04-28 PM). 3 tests driving real Home Oil PDF (`tests/fixtures/ODBIN-0195942_2.PDF`, 54 KB committed) through pdfjs worker → parseFuelBillPdf → modal preview → Save → Supabase rows + storage. Harness additions: idempotent `createBucket('fuel-bills')` setup test in `tests/setup/global.setup.js`; Storage API recursive cleanup helper in `tests/setup/reset.js` (replaces blocked SQL approach — see §8 Known gotchas). Zero production diff.
   - **A8b fuel reconciliation UI spec** — **NEXT** (Codex-approved 2026-04-28 PM, scope locked). 4 tests covering green / orange / red variance bands plus the §7 cell-destination exclusion rule. **Production patch** (~5 lines): add three data attributes on each variance cell in `FuelReconcileView.jsx:201-203` — `data-variance-band="green|orange|red"`, `data-fuel-type="diesel|gasoline|def"`, `data-month="YYYY-MM"`. Per-cell (not per-row) because diesel / gas / DEF can each have different bands within the same month. Selectors stay stable without relying on table column positions. **Seed**: one fixed readable month (e.g. `2026-01`); one minimal `equipment` row per spec (fields `id`, `name`, `slug`, `category`, `tracking_unit`, `status:'active'` — that's enough for the `equipment_fuelings.equipment_id` FK). Each test seeds one deterministic gallons combo for the target band; reset between tests. **Assertions**: visible purchased + consumed gallons per Codex's "catch math regressions where the band coincidentally stays the same"; band attribute via the data-* hook; signed variance text (e.g. `-2.0%`) asserted in at least one test. **Test 4 (cell exclusion)**: seed `fuel_supplies.destination='cell'` row whose inclusion would push variance from 2% to ~48%; assert the cell stays green to lock §7 explicitly. **Skip positive-variance inversion** — `varColor()` uses `Math.abs(pct)` so positive/negative is not a separate band branch; sign is in display text only. **Open questions** are minor (CC plan packet has 3, all small selector/seed details that resolve at implementation time).
   - **A10 CI integration** — GitHub Actions running lint + vitest + playwright on every PR. **Deferred behind Initiative B** (lint baseline must be clean before CI surfaces noise).
4. **ESLint + Prettier** (Initiative B) in a separate focused pass. Unblocks A10. Will surface cleanup noise; keep it isolated from feature work.
5. **PWA shell / mobile install** (Initiative C). Lowest urgency. Useful but doesn't protect data workflows.
...
- **Additional Playwright coverage beyond A8b.** 8 of 9 planned specs shipped 2026-04-28 (see §8 sequencing). Beyond A8b + A10 (CI), candidates as the app grows: equipment fueling webform flows, public weigh-ins webform, layer housing math, Add-Feed webform.
...
- **DOM hooks added 2026-04-28 PM for stable Playwright selection** (no logic change, no semantic meaning beyond test infrastructure): `data-gantt="1"` + `data-week-header="1"` + `data-iso={isoString}` + `data-today-line="1"` on `BroilerTimelineView.jsx`. A8b will add `data-variance-band="green|orange|red"` + `data-fuel-type="diesel|gasoline|def"` + `data-month="YYYY-MM"` on `FuelReconcileView.jsx`. If a future refactor renames or removes these attributes, the corresponding Playwright spec breaks — search for `data-` in `tests/` before refactoring view files.
