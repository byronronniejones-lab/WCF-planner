...
**Last consolidated:** 2026-04-21 session 3 (post-polish cutover). **Last session-index update:** 2026-04-28 (eve) (Feed physical count delivery-included flag).
...
- ~~**Feed physical count: delivery-included flag.**~~ **DONE** 2026-04-28 (eve). Persists `inv.includesCurrentMonthDelivery: bool` on pig + per-poultry-type count records. When true, the count's-own-month order is suppressed from the ledger END math AND the system-side count-adjustment compare (so a perfect count shows zero phantom Adj). Touched `src/pig/PigFeedView.jsx` + `src/broiler/BroilerFeedView.jsx`. Default false; old records read as false. No DB migration. No §7 entries touched. Codex review: 3 rounds. Pre-commit gate: 68/68 vitest, 0 errors / 636 warnings, clean build.
- **No new build queued.** Pick from §8 Near-term or Deferred Initiative B Phase 2.4-2.6 next session.
