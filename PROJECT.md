...
**Last consolidated:** 2026-04-21 session 3 (post-polish cutover). **Last session-index update:** 2026-05-06 (Tasks v1 C1-C4 shipped through mig 046; Cattle Forecast plus broiler/cattle hotfixes captured in §8 + Part 4). HO.md remains the static start-of-session SOP; PROJECT.md carries project state and roadmap only.
...
Current parent-path migrations run through `046_tasks_weekly_summary.sql`. Key recent anchors: migs 030/031 offline queue + daily photos, migs 034/035 Add Feed + weigh-in RPCs, migs 036-046 Tasks v1, migs 043/044 Cattle Forecast + calving automation, and mig 045 pg_net timeout bump for task cron.
...
| `tasksAdminApi.js`, `tasksPublicApi.js`, `tasksUserApi.js` | Side-effect wrappers for admin Tasks Center, public `/webforms/tasks`, and logged-in `/my-tasks` surfaces. |
...
| `/equipment`, `/equipment/fleet`, `/equipment/fuel-log` | equipment + fuel log | auth |
...
| `/webforms`, `/addfeed`, `/weighins`, `/webforms/tasks`, `/fueling`, `/fueling/<equipment-slug>`, `/fuel-supply` | public webforms | **no auth** |
...
### Recently shipped (2026-05-01 through 2026-05-06)

- **Tasks v1 C1-C4 closed.** Admin Tasks Center, `/my-tasks`, public `/webforms/tasks`, request photos, completion photos, daily generation, weekly summaries, migs 036-046, and TEST/PROD function+DB probes are shipped through `5c839e7`.
...
No active approved build is in progress after C4. Current build candidates should be chosen from work that is not already built and verified:

- **Equipment Material List.** Add structured parts metadata to equipment service tasks and build an admin material-list view.
- **Equipment Reading Reconciliation.** Resolve parent `equipment.current_hours/km` drift from public fueling submissions without broad anon UPDATE on `equipment`.
- **More Podio imports.** Inventory each app, design mappings, dry-run, and audit against exports before importing.
- **CI Actions secrets + first green e2e workflow.** Configure the five repo secrets and re-run CI to prove the shared TEST database guard path.
...
### Initiative C shipped contract

- PWA/manifest foundation is shipped; no service worker is installed yet.
- Offline queue direct-insert flows use plain `.insert()` plus unique `client_submission_id` 23505-as-synced handling under anon RLS. RPC-mediated flows use explicit registry entries in `offlineRpcForms.js`.
- Daily report photos are shipped for pig, broiler/poultry, layer, cattle, and sheep daily reports. Egg remains intentionally excluded.
- Pig and broiler fresh draft weigh-in creation routes through `submit_weigh_in_session_batch`; cattle/sheep and completion/processor side effects remain online-direct until a dedicated RPC design exists.

### Shipped locks + queued build notes

This section keeps shipped contracts close to the roadmap only when they shape future work. It should not list already-shipped work as an outstanding build.

**Daily Report Photos (shipped):**
- Scope shipped: pig, poultry/broiler, layer, cattle, sheep. Egg is excluded.
- Storage: private `daily-photos` bucket. App stores paths; admin reads via signed URLs.
- Compression target: 1024px max edge / JPEG 0.7 / roughly 80KB target. Max count is locked by `MAX_PHOTOS_PER_REPORT`.

**Cattle Herd filters + maternal UI scrub (shipped):**
- Herd filter/sort rebuild, grouped/flat toggle, calf-count filtering/sorting, and local smart-filter assistant shipped in `51d612e`; summary/herd sorting tightened in `fd77ee1`.
- Cattle maternal issue UI reads/writes are retired. DB column drops remain future DB cleanup only after explicit TEST/PROD verification.

**Cattle Forecast tab (shipped 2026-05-04):**
- Forecast source shipped in `d2eccda` with mig 043; PROD and TEST both have mig 043 applied.
- Real `cattle_processing_batches.status` values are only `active` or `complete`; "planned" is a Forecast/Batches display concept.
- Mig 044 extends calving automation: heifer with calf -> cow; PREGNANT cow/heifer with calf -> OPEN.

**Equipment Material List (queued):**
- Add structured `parts_required` metadata to each `equipment.service_intervals[].tasks[]` entry.
- Build admin material list view for due-within-100-hours hour-based equipment; Hijets include all material-tagged maintenance regardless of distance window.
- Clearable like missed reports; clear expires when relevant interval state changes.

**Tasks Module v1 (shipped):**
- Assignee = `profiles.id` login user. Submitted-by for public tasks = team-roster display-name string.
- Public submit is one-time only. Recurrence exists on admin templates only.
- Daily cron schedule: `0 4 * * *` UTC. Weekly summary schedule: `0 13 * * 1` UTC.
- No required-photo behavior. Completion and request photos are optional and append-only.
...
| 2026-05-06 | `267e877`..`5c839e7` + this wrap | **Tasks v1 C1-C4 closed; broiler/cattle hotfixes captured.** Admin Tasks Center (`267e877`) shipped, C1.1 removed the cron button and clarified the one-time/recurring modal (`0f8aafb`), C3 public `/webforms/tasks` and mig 041 shipped (`5208ece`), repeat/default-inclusion hotfixes landed (`9e86633`, `1475b06`), request photos + mig 042 shipped (`9c2091b`), broiler batches auto-activate on hatch date (`fd12e6a`), cattle heifer inclusion states were clarified (`abc720f`), C2 `/my-tasks` + mig 040 shipped after TEST/PROD verification (`9a0cdfc`), and C4 weekly summary + mig 046 shipped after TEST/PROD Vault/function/migration/probe verification (`5c839e7`). This docs wrap refreshes HO as the static start-of-session SOP and updates PROJECT so shipped Tasks/Cattle/Broiler work is not listed as future build scope. |
