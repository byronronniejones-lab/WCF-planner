...
- **Admin broiler session metadata edit + `recomputeBroilerBatchWeekAvg` helper** (added 2026-04-30, commit `00754c3`). `/broiler/weighins` expanded broiler rows surface an always-visible inline metadata-edit panel (broiler-only; pig sessions in the same view do not show it; cattle/sheep are routed through different views). Admin can change `broiler_week` (WK 4 / WK 6 toggle) and `team_member`. **Side-effect contract:** when status === 'complete' AND `broiler_week` changes, `app_store.ppp-v4[batch].wk*Lbs` for the OLD week is recomputed/cleared via `recomputeBroilerBatchWeekAvg(sb, batchId, oldWeek, {excludeSessionId: s.id})`, then the NEW week's avg is stamped via the existing `writeBroilerBatchAvg`. Draft sessions and team-only edits do NOT touch ppp-v4 (matches today's `writeBroilerBatchAvg` complete-only gate). **Helper contract** (`src/lib/broiler.js`): `{ok: true|false, message?}`. `{ok: true}` covers successful recompute, successful delete, OR intentional no-op (no usable entries on the picked session, batch row not found in ppp-v4, ppp-v4 row missing). `{ok: false, message}` ONLY for actual Supabase read/upsert errors. `saveSessionMetadata` only treats `{ok: false}` as a user-visible save failure. **`excludeSessionId` is mandatory** for the admin path so the moved session can't win its own old-week recompute (the `.neq('id', excludeSessionId)` is skipped when omitted/null for general-purpose use). **DELETE semantics on no-other-session:** the helper does `delete next[fieldKey]` on the batch row — clean "field absent" rather than null, matches "no saved average exists" intent. **Last-write-wins per (batch, week) preserved:** when multiple complete sessions exist, the latest by `completed_at` wins (matches `writeBroilerBatchAvg`'s single-session aggregation today). **Legacy team_member preservation:** if `s.team_member` is no longer in the active roster, the dropdown injects it as `'<name> (retired)'` so historical sessions don't blank-render or force a team change on save. The retired option is per-session — switching TO a retired name from a different one is not possible. **`writeBroilerBatchAvg` signature unchanged**; only the new helper is "loud" (returns `{ok, message}`). Other callers (`completeFromAdmin`, `saveAdminGrid`, `completeSession`, `finalizeSession`) keep today's quiet semantics. **Out of scope:** reopening a complete broiler session (status complete → draft) does NOT clear/recompute `ppp-v4` `wk*Lbs` — same staleness class, deliberately deferred (see §8 follow-ups).
...
- **Initiative C — Mobile install + offline/photo-capable webform queue.** Plan-approved 2026-04-28 (eve+). Phase 1A (DB schema contracts, mig 030 applied) shipped 2026-04-28 (eve queue, commit `9e93e0a`). Phase 1B (adapter foundation + FuelSupply canary) shipped 2026-04-29 (commit `b5a07a9`). Migration 031 (`daily-photos` bucket) still pending manual SQL Editor apply against prod — needed before Phase 2 wires photo capture; Phase 1B canary doesn't need it. See "Initiative C v1 plan capture" below for full scope.
...
**Phase plan:**
- ~~**Phase 1A — DB schema contracts.**~~ **DONE** 2026-04-28 (eve queue, commit `9e93e0a`). Mig 030 (CSID + photos columns) + 031 (daily-photos bucket + policies). Mig 030 applied to prod 2026-04-29; mig 031 still pending manual SQL Editor apply (Phase 2 dependency, not Phase 1B).
- ~~**Phase 1B — adapter foundation + canary.**~~ **DONE** 2026-04-29 (commit `b5a07a9`). Shipped: `idb` + `fake-indexeddb` deps, `src/lib/{clientSubmissionId,offlineForms,offlineQueue,photoCompress,useOfflineSubmit}.js` (5 modules + tests, 49 vitest cases), `src/webforms/StuckSubmissionsModal.jsx`, FuelSupplyWebform rewired with dual synced/queued copy, PWA `manifest.webmanifest` + 192/512 placeholder icons + index.html links (no Service Worker yet — Phase 3). Background sync = `online` event + 60s tick + manual button. 3-retry budget, then stuck-modal surface. `recoverStaleSyncing(formKind, {staleAfterMs: 30_000})` resets orphaned in-flight rows on the next pass (closes the tab-crash limbo state Codex flagged). **Anon insert + 23505 idempotency** (revised from the Phase 1A upsert plan — RLS reason captured in §7 + at the top of `useOfflineSubmit.js`). Playwright: `offline_queue_canary.spec.js` (3 tests under anon context) + `offline_queue_dedup.spec.js` (4 tests covering service-role upsert, distinct csids, NULLS DISTINCT, anon insert + 23505 + the upsert-fails-as-anon negative lock).
- ~~**Phase 1C-A — AddFeed parent-aware RPC offline queue.**~~ **DONE** 2026-04-29. AddFeed got its own offline path via the `submit_add_feed_batch` RPC (mig 034) — parent + N children atomically per submission. New module `src/lib/offlineRpcForms.js` (RPC-form registry, sibling to `offlineForms.js`'s flat-insert registry); new hook `src/lib/useOfflineRpcSubmit.js` mirrors `useOfflineSubmit`'s lifecycle but the submit path is `sb.rpc(record.rpc, record.args)`. AddFeedWebform's three submit branches (broiler/pig/layer multi-row, cattle, sheep) now flow through `useOfflineRpcSubmit('add_feed_batch')`. Stable IDs across retries: csid + parentId minted once at submit-entry; child IDs are deterministic from parentId + index (`${parentId}-c${i}`) so retry produces byte-identical RPC args. **23505 from the RPC path is NOT treated as success here** (unlike the flat-insert hook's anon-friendly idempotency) — the RPC's own `ON CONFLICT DO NOTHING + fallback SELECT` makes 23505 a real bug if it surfaces, classified as schema-error → stuck modal. Idempotent replay arrives as `{data: {idempotent_replay: true}, error: null}` and falls into the no-error → markSynced path naturally. `localStorage.wcf_team` is written on BOTH `synced` and `queued` outcomes (queued = captured-on-device successfully). StuckSubmissionsModal copy makes clear that discarding drops an unsynced AddFeed submission. Tests: 20 vitest in `useOfflineRpcSubmit.test.js` (registry shape + classifier + queue persistence + idempotent_replay path); 4 Playwright cases in `offline_queue_multi_form.spec.js` (online happy path, offline → queued IDB record with rpc shape, recovery on reload, idempotent replay against pre-seeded csid). Phase 1C-B (WeighIns + PigDailys via flat-insert path) remains follow-up — those forms continue to use direct `.insert()` until a future build wires them. **No service worker, no photos, no schema changes.**
- ~~**Phase 1C-B — PigDailys no-photo flat queue.**~~ **DONE** 2026-04-29. Adds `pig_dailys` to `offlineForms.js` (flat-insert registry; `hasPhotos: false`). PigDailysWebform routes through `useOfflineSubmit('pig_dailys')` ONLY when `wfPhotos.length === 0`; the photo-attached path stays fully online-only with explicit "Photo submission could not be saved … photo submissions need a connection — remove photos to save offline, or try again." copy on either upload OR insert failure. Photo-attached submissions never enter IDB regardless of failure mode (locked by `tests/pig_dailys_offline.spec.js` Test 4). Row shape preserved exactly: NO `source` field (current PigDailys rows omit it), NO `feed_type` (pig_dailys lacks the column — §7), `photos: []` literal. 23505 replay still recognized as synced through the flat hook's existing anon-friendly idempotency (Test 5). StuckSubmissionsModal added; `describeRow` reads from `row.record` (authoritative replay object) rendering `<batch_label> · <date> (not yet sent)`. `wfReset()` clears `wfDoneState='none'` + photos + statuses. `localStorage.wcf_team` written on BOTH synced and queued outcomes. **Latent ReferenceError fix**: pre-1C-B `wfSubmit` referenced `wfCfg.fields` directly which was undefined under the default `sections[].fields[]` config shape; flattened inline. Tests: 9 new pig_dailys cases in `offlineForms.test.js` (no source, no feed_type, photos:[], stable replay, falsey-value pass-through), 5 Playwright cases in `pig_dailys_offline.spec.js`. **No service worker, no schema/RLS/migration changes, no WeighIns work.**
- ~~**Phase 1C-C — WeighIns session-aware offline RPC precursor.**~~ **DB/RPC piece DONE** 2026-04-29. Mig 035 ships `submit_weigh_in_session_batch(parent_in jsonb, entries_in jsonb)` — SECURITY DEFINER + anon EXECUTE, mirrors mig 034's race-safe idempotency contract. v1 species allowlist enforced in-function: `pig` + `broiler` only (cattle/sheep deferred). v1 status: `'draft'` only (completion stays online). Required-field validation with clear pre-insert RAISEs (date, team_member, entries non-empty, broiler_week ∈ {4,6} for broiler / coerced NULL for pig). Children carry NULL `client_submission_id` (parent owns dedup). No side-effect column writes (transfer flags, processor flags, prior_herd_or_flock, sent_to_trip_id) — runtime concerns deferred. **No runtime form code wired** in this build — `WeighInsWebform.jsx` UNCHANGED; existing direct-INSERT flows continue. 17 Playwright cases lock the RPC contract (`tests/weigh_in_session_batch_rpc.spec.js`). See §7 entry above for the full contract. **Mig 035 still pending manual prod apply** before any runtime cutover ships. **Runtime cutover is a separate future build** (Phase 1C-C+) with its own scope packet — explicitly out of scope for this DB-only build.
- **Phase 1D — daily-report photo offline queue (queued, larger scope).** PigDailys photo offline support — and the same for the other 4 daily-report forms (broiler / layer / cattle / sheep dailys) — belongs here, not in 1C-B. Needs the IDB `photo_blobs` store wiring (already declared in `offlineQueue.js` Phase 1B but unused), a photo replay path, and the `daily-photos` storage bucket (mig 031 applied to test, still pending manual prod apply per HO.md).
- **Phase 2 — first photo-capable form.** Wire EquipmentFuelingWebform photo flow against the new `daily-photos` bucket (apply mig 031 before this starts). Switch its existing photo path scheme from `Date.now()-based` to deterministic `<form_kind>/<client_submission_id>/<photo_key>.jpg`. Don't migrate historical photos — leave existing prod paths untouched (the photo→fueling matcher already keys on `(equipment_id, date)`, not path). ~1 session.
- **Phase 3 — daily-report photo capture + Service Worker shell.** UI for photo capture on **5 daily-report forms only: pig, poultry, layer, cattle, sheep** (egg explicitly excluded per Ronnie). SW caches app shell + webform routes. Cache strategies for `webform_config` reads (cache-first, revalidate hourly). Optional install prompt UX. ~1–2 sessions.

**Test plan (each phase's gates):**
- Vitest: `clientSubmissionId.test.js`, `offlineForms.test.js`, `offlineQueue.test.js` (incl. recoverStaleSyncing 5 cases), `photoCompress.test.js`, `useOfflineSubmit.test.js`. **All shipped Phase 1B.**
- Playwright: `offline_queue_canary.spec.js`, `offline_queue_dedup.spec.js` shipped Phase 1B. Phase 1C: `offline_queue_multi_form.spec.js`. Phase 2: `offline_photo_upload.spec.js`, `offline_photo_dedup.spec.js`, `offline_storage_quota.spec.js`. Network blocking via Playwright route stubs.

### Next build

- **Team Member Master List Cleanup.** The current team-member story is fragmented: multiple Add buttons across admin views, the public Fuel Supply form's selection drops/overwrites mid-edit, the Sheep weigh-in admin selector is broken, and `webform_config.team_members` + `per_form_team_members` get clobbered by stale read-then-write patterns despite the §7 "read-fresh-then-write" rule. Locked decisions captured in "Locked decisions for queued builds" below; full plan-packet expected before code per the SOP. Files CC must inspect first: `src/webforms/{FuelSupplyWebform,WeighInsWebform,AddFeedWebform,WebformHub,PigDailysWebform,WebformsAdminView}.jsx`, `src/admin/EquipmentWebformsAdmin.jsx`, plus a `team_members|teamMembers|per_form_team_members|wcf_team` ripgrep sweep.
...
**Team Member Master List Cleanup (next build):**
- Canonical source stays `webform_config.team_members`, but reshape from `string[]` to `[{id, name, active}]`. Read-side helpers normalize old `string[]` rows on the fly during transition.
- Historical rows (`*_dailys`, `weigh_ins`, `equipment_fuelings`, `fuel_supplies`) keep storing display-name strings. **Don't rewrite history.** New webform submissions can still store `team_member: name` for compatibility.
- Tasks (later) will assign to `profiles.id`, NOT team-member IDs. Different identity space.
- No per-form filtering for now. Every active team member appears everywhere unless Ronnie reverses this. Inactive names stay available for admin/history rendering but hidden from new public dropdowns.
- Remove every "Add team member" button outside the central admin editor (which lives in admin/webforms/team-member management). Public webforms become read-only consumers of the master list.
- Fix Fuel Supply unselecting via stomp-prevention (read-fresh-then-write everywhere; no stale React state writes to the master JSONB). Fix Sheep weigh-in admin selector as part of the shared contract.
- Don't broaden RLS. Don't change Supabase auth config. Don't touch Phase 1B offline queue files unless this build directly needs the shared team-member helpers (it shouldn't).
- Tests: vitest for the helper normalization + active-filter behavior; Playwright/admin smoke if cheap.
...
**Add Feed / Parent Submission Design (Phase 1C precursor):**
- Don't bolt photos onto generated `*_dailys` rows ad-hoc. Add Feed produces N daily-row inserts per submission (one per batch); a flat queue row per insert breaks photo attribution (which photos belong to which child row?) and breaks atomic submit semantics (some children land, others don't).
- Preferred model: new `daily_submissions` parent table with its own `client_submission_id`. Normal daily forms = 1 parent → 1 child. Add Feed = 1 parent → N children.
- Preferred submit path: RPC / server-side transaction so parent + children insert atomically.
- Implication: this design must land BEFORE Add Feed gets wired into the offline queue. Plan-packet first.

**Cattle Improvements (queued):**
- Momma herd cow tile shows derived calf count.
- Heifer auto-promotes to "cow" status on `cattle_calving_records` insert.
- Prefer DB trigger + one-time backfill for the heifer→cow flip, plus a `cattle_comments` audit entry on each promotion. Avoids client-side promotion logic that would silently drift if multiple paths create calving records.
...
- Recurrence: anchored from original due date. Future recurring instances are generated automatically 3 days before their due date. Missed/overdue instances stack (no auto-skip). Completed tasks remain visible in history.
- Optional/required completion-photo support (per-task config); some tasks may require a photo on completion.
...
- **Required before prod ship:** scheduled generator (server-side cron / edge function) that creates the next-3-days instances. App-open generator alone is not enough — users who don't open the app would never see new instances appear.
...
- **Reopen-session broiler avg cleanup** (deferred from 2026-04-30 admin metadata-edit lane). When admin reopens a complete broiler session via `reopenSession` (status flips complete → draft), `app_store.ppp-v4[batch].wk*Lbs` is NOT cleared or recomputed. The stored avg lingers until another complete session for that batch+week stamps a new value, or until the batch's complete session is edited via the metadata panel (which has the helper contract). Same staleness class as the WK-change case the metadata-edit lane fixed. Natural fix: call `recomputeBroilerBatchWeekAvg(sb, batchId, oldWeek, {excludeSessionId: s.id})` from `reopenSession` for broiler sessions. Plan-packet first per SOP.
