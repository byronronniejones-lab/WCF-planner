...
- **Immediate operational lane: Tasks Phase B PROD deploy** (P1-P7 playbook). Phase B is source-shipped on `main` (commit `a44971a`) and TEST-verified end-to-end, but PROD has none of mig 039 / `tasks-cron` Edge Function / Vault secrets / cron schedule yet. The deploy is manual, in this order: (P1) provision PROD Vault secrets via SQL Editor — `TASKS_CRON_FUNCTION_URL` (the prod Edge Function URL), `TASKS_CRON_SECRET` (freshly minted via `gen_random_bytes(48)`; NEVER reuse the TEST value), `TASKS_CRON_SERVICE_ROLE_KEY` (PROD legacy service-role JWT). (P2) `supabase functions deploy tasks-cron --project-ref pzfujbjtayhkdlxiblwe`. (P3) `supabase secrets set --project-ref pzfujbjtayhkdlxiblwe TASKS_CRON_SECRET=<...> TASKS_CRON_SERVICE_ROLE_KEY=<...>`. (P4) Apply mig 039 in PROD SQL Editor. (P5) Manual SQL Editor verification ONLY — no `exec_sql` in PROD; walk all three audit layers via SELECT (`SELECT public.invoke_tasks_cron();` → `SELECT * FROM net._http_response ORDER BY created DESC LIMIT 3;` → `SELECT * FROM task_cron_runs ORDER BY ran_at DESC LIMIT 3;`). (P6) `node scripts/probe_tasks_cron_function.cjs` against the PROD URL with `TASKS_CRON_SECRET` env var (Ronnie copies once from PROD Vault; the script's auth-only probes are PROD-safe per Codex Q4). (P7) Confirm `cron.job` registered + `tasks-cron-daily` next_run set; wait for first 04:00 UTC fire and re-walk the audit layers.
- **Next source build after PROD deploy: Tasks Phase C — admin Tasks UI.** Gated behind PROD Phase B verification. Scope per the queued plan in this section: admin-only tab/view to create/edit/disable templates, view active task_instances, see cron-run audit footer (`SELECT 1 FROM task_cron_runs ORDER BY ran_at DESC`), and run the cron generator manually via the function's admin-mode probe-false path. NO `/my-tasks` (Phase D), NO public submit form (Phase E), NO weekly summary (Phase F) in Phase C. Plan packet first.
