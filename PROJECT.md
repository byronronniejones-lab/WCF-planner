...
- **Phase 1C-B — fan-out across remaining flat-insert text forms** (queued). Wire WeighInsWebform (alongside existing drafts) + PigDailysWebform via the existing `useOfflineSubmit` flat-insert hook. Each = one registry entry in `src/lib/offlineForms.js` + hook call. ~1 session.
