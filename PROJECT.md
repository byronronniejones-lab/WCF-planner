...
- **Podio equipment photos pull — rate-limited, needs resume logic.** `scripts/pull_podio_equipment_photos.cjs` was started 2026-04-23 and burned through Podio's 5,000 req/hr quota after walking the 1,080-item Fuel Log app (makes 1 `/item/{id}` request per item to fetch the `files` list, plus comment fetches). Got throttled with `420 You have hit the rate limit. Please wait 3600 seconds`. Script needs: (a) detect 420 response and back off for the suggested duration (or until next hour), (b) checkpoint progress to `_photos_pull_cursor.json` keyed by `(app_id, item_id)` so a fresh run resumes where it stopped, (c) consider bulk `/item/app/{app_id}/filter/` with `fields=files` if supported to batch, (d) throttle proactively to stay under ~4,000 req/hr. Once it finishes, `--upload` writes to `equipment-maintenance-docs` Storage + creates `equipment_maintenance_events` rows + patches `equipment_fuelings.photos`. **This is the reason Podio-imported fueling rows show no photos on the planner** (new webform submissions upload directly to Supabase Storage and do show). Top near-term item.
