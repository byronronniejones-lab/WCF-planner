...
### Recommended sequencing (set 2026-04-27, updated 2026-04-28)

Priority order for the next session. Items 1–2 shipped 2026-04-27. Item 3 (Playwright) is in progress: A2 harness + A4 first domain spec landed 2026-04-28, expansion to other workflows queued.

1. ~~**Cattle/sheep processor workflow.**~~ **DONE** (commits `f1adb81` Phase 1 + `448152e` flag-clear fix + `802f393` Phase 2 + `6d70669` Phase 3 + `62e1064` gate loosen).
2. ~~**Pig FCR cache.**~~ **DONE** (commit `a8c7133`).
3. **Playwright integration tests** — IN PROGRESS.
   - ~~**A2 harness**~~ **DONE** (commit `0ad8fc2`). Local-only Playwright runner against an isolated Supabase test project. `assertTestDatabase` guard refuses prod URL or missing flag. `tests/setup/reset.js` truncates a 29-table whitelist via `exec_sql` RPC. `scripts/build_test_bootstrap.js` generates a 99 KB bundle for fresh-project setup (9 hand-created table seeds + 29 migrations + test-only `exec_sql`). Smoke spec: login + storageState + backend sentinel (asserts `window.__WCF_SUPABASE_URL` contains the test project ref before login).
   - ~~**A4 pig batch math spec**~~ **DONE** (commit `3234ff6`). One happy-path Send-to-Trip test covering subAttributions schema, ledger current count math, and lbs/pig finishers-denominator regression for the 1644 vs 1186 P-26-01A bug.
   - **A5 cattle Send-to-Processor spec** — NEXT. Drive `prior_herd_or_flock` stamping + the 4 detach paths (toggle clear, entry delete, session delete, batch delete) through the UI. Touches §7 heavily — review at PLAN time before writing code.
   - **A6 sheep Send-to-Processor spec** — mirror of A5 but covers the looser gate (any-flock, not finishers-only).
   - **A7 broiler timeline spec** — range derives from data, auto-scrolls to today.
   - **A8a fuel bill PDF parser spec** — real PDF fixture (`ODBIN-0195942_2.PDF`) → `parseFuelBillText` end-to-end.
   - **A8b fuel reconciliation UI spec** — seeded fuel_bill_lines + equipment_fuelings + fuel_supplies asserting variance bands.
   - **A9 FCR cache spec (Edit Trip)** — followup from A4. Drives `PigBatchesView.persistTrip` via Edit Trip → Save to verify `parent.fcrCached` populates on add and clears via `delete next.fcrCached` on null.
   - **A10 CI integration** — GitHub Actions running lint + vitest + playwright on every PR. Deferred per Codex review until lint baseline is clean (Initiative B).
...
- **Playwright integration tests** (Vitest unit harness shipped 2026-04-27 — covers `src/lib/equipment.js`, `src/lib/dateUtils.js`, `src/lib/routes.js`, `src/lib/fuelBillParser.js`. Browser-flow coverage still pending).
