...
**Last consolidated:** 2026-04-21 session 3 (post-polish cutover). **Last session-index update:** 2026-04-28 (late PM) (Playwright Phase 1 wrapped + Initiative B Phase 1 + A10 CI + equipment dashboard rollup).
...
6. ~~**Equipment dashboard rollup** (`a2159b5`, 2026-04-28 late PM)~~ + ~~**A1 regression spec** (`0f57d47`, 2026-04-28 late PM)~~. HomeDashboard EQUIPMENT ATTENTION gains `upcoming` (next_due within 50h/km) and `missed_fueling` (>14 days since last fueling) alerts; existing `overdue` / `fillup_streak` / `warranty` kinds unchanged. Auto-clear semantics on the new kinds (no Clear button); only `warranty` remains manually clearable. 5 Playwright tests (one per alert kind). Added `data-attention-kind` + `data-equipment-slug` DOM hooks. DEF-low warnings deferred (no current-level data model).

### Deferred Initiative B follow-ups (Phase 2.4-2.6)

Lint at 0 errors closed Initiative B's hard milestone. Remaining cleanup is mechanical churn that doesn't unlock anything:

- **Phase 2.4 Prettier autofix.** 202 files would be reformatted by `npm run format`. Suggest splitting in chunks (src/ → tests/ → scripts/) when picked up to keep blame readable. Once landed, A10 can add `npm run format:check` as a required step.
...
- **Initiative C — PWA shell / mobile install.** Lowest urgency. Useful but doesn't protect data workflows.

### Next build (queued 2026-04-28 late PM)

- **Feed physical count: delivery-included flag.** Codex-scoped, plan-packet-first. When entering a physical feed count (pig or poultry), add a shared question: "Does this count include this month's feed delivery?" One delivery per month covers all feed (not per feed type), so the flag is per-count not per-feed-type. Persist on the count record: `{count, date, includesCurrentMonthDelivery}` for both pig and poultry count entries. **If checked**, do NOT add the current month's order again after the count. **If unchecked**, keep current behavior: the current month's order arrives at month-end and is added later. Do NOT attempt to infer delivery date — delivery day varies month-to-month. **Plan-packet first**: read `src/pig/PigFeedView.jsx` + `src/broiler/BroilerFeedView.jsx`; explain exactly which formulas change for Actual On Hand, End of Month Est., suggested order, and monthly ledger badges. Keep narrow: no UI redesign, no feed-order model rewrite.
...
- **HomeDashboard equipment alerts: "never logged" follow-up.** Pieces with zero fueling history don't currently surface a `missed_fueling` row because the `latestByDate` reduce returns null. Codex flagged as a non-blocking follow-up; not added in the rollup commit because it's a behavior decision, not regression coverage. If Ronnie wants zero-history equipment to alert (e.g. as `kind:'no_fueling'`), it's ~10 lines + 1 test atop the existing pattern.
- **HomeDashboard equipment alerts: sort-order + auto-clear-on-resolve specs.** A1 (`0f57d47`) covers each of the 5 alert kinds in isolation. Two follow-ups remain uncovered: (a) sort-order across multiple kinds in one seed, asserting overdue → upcoming → missed_fueling → fillup_streak → warranty; (b) auto-clear semantics — e.g., overdue row vanishes after the interval is ticked complete on a fueling. Both are deferrable scope.
...
- **DOM hooks added 2026-04-28 for stable Playwright selection** (no logic change, no semantic meaning beyond test infrastructure):
    ...
    - `HomeDashboard.jsx`: `data-attention-kind="overdue|upcoming|missed_fueling|fillup_streak|warranty"` + `data-equipment-slug="<slug>"` on each EQUIPMENT ATTENTION row (shipped late PM, A1 follow-up).
