...
- **Phase 1C-C — WeighIns session-aware offline (design packet, no code yet).** WeighInsWebform is NOT a simple flat insert: it creates `weigh_in_sessions` drafts, inserts `weigh_ins`, runs broiler grid delete-then-reinsert in `saveBatch()`, completes sessions, runs cattle/sheep processor side effects, and runs cattle create/retag/reconcile/comment side effects. Full offline support requires a session-aware design — likely a new SECURITY DEFINER RPC analogous to `submit_add_feed_batch` plus stable IDs for sessions and entries, plus a way to replay session edits idempotently. Build a design packet first; no code in this build.
