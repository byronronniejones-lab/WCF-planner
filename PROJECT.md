...
| `task_templates`, `task_instances`, `task_cron_runs` | Tasks Module v1 Phase A (migs 036-038, shipped 2026-05-01) + Phase B (mig 039 + tasks-cron Edge Function, source `a44971a`; PROD deploy P1-P6 green 2026-05-01, P7 = first scheduled cron fire 23:00 CDT 2026-05-01 pending). Phase B drops `requires_photo` from both task tables, extends `recurrence` enum to include `quarterly`, installs `pg_cron`/`pg_net`/`pgcrypto`, and stands up the daily 04:00 UTC generator. Templates are admin-only. Instances are admin-all plus assignee SELECT-own. Cron runs are admin SELECT only and service-role append-only. UI / completion RPC / public submit / weekly summary land in Tasks v1 combined-build (C1-C4) — see §8 Next build. Future migs: 040 (C2 complete RPC), 041 (C3 public-submit RPCs), 042 (C4 summary + new `task_summary_runs` table). |
...
| `task-photos` | Private bucket from mig 038. NO required-photo behavior anywhere in Tasks v1. Optional completion photos are LOCKED for C2 (assignee-only authenticated upload at completion; never required). Bucket stays `public:false`; reads via signed URLs only. |
...
- **Tasks Module v1 Phase A RLS contract** (migs 036-038, commit `4874f1d`, shipped 2026-05-01). `public.is_admin()` is the single admin-role helper for task RLS and future task RPCs. It is `SECURITY DEFINER`, `STABLE`, `SET search_path = public`, and checks `profiles.id = auth.uid()` with `role = 'admin'`. Grant pattern is deliberate: `REVOKE ALL FROM PUBLIC` + `GRANT EXECUTE TO authenticated`, with no explicit `REVOKE EXECUTE FROM anon`; anon may call and receives `false` because `auth.uid()` is NULL. A stricter anon revoke caused Supabase/PostgREST schema-cache/auth sign-in failures during recon, so do not reintroduce it without a fresh failing/proving test. `task_templates` are admin-only. `task_instances` are admin-all plus assignee SELECT-own; no direct assignee UPDATE. `task_cron_runs` are admin SELECT only; cron/admin generation writes through service role. The private `task-photos` bucket exists. NO required-photo behavior anywhere in Tasks v1. Optional completion photos are LOCKED for the Tasks v1 combined-build C2 (assignee-only authenticated upload at completion via mig 040's `complete_task_instance` RPC; `completion_photo_path` carries the canonical bucket-prefixed path). Bucket stays `public:false`; reads via signed URLs only.
- **Tasks Module v1 Phase B contract** (mig 039 + tasks-cron Edge Function, commit `a44971a`, source-shipped 2026-05-01; PROD deploy P1-P6 green 2026-05-01, P7 pending first scheduled cron fire 23:00 CDT 2026-05-01). `requires_photo` is DROPPED from both `task_templates` and `task_instances`. `task_instances.completion_photo_path` and the `task-photos` bucket are NOT dormant — optional completion photos are LOCKED for the Tasks v1 combined-build C2 (mig 040 `complete_task_instance` RPC owns assignee-or-admin path-shape validation; canonical path = `task-photos/<assignee_uid>/<instance_id>/<filename>`). The recurrence CHECK constraint on `task_templates.recurrence` is extended to include `quarterly` (full enum: `once|daily|weekly|biweekly|monthly|quarterly`); `task_templates_recurrence_interval_check` (`>= 1`) is preserved untouched. **Vault contract** uses `vault.decrypted_secrets` (NOT `vault.read_secret`) with three locked names: `TASKS_CRON_FUNCTION_URL`, `TASKS_CRON_SECRET`, `TASKS_CRON_SERVICE_ROLE_KEY`. Mig 039 RAISEs at apply-time if any secret is missing/empty. **`public.invoke_tasks_cron()`** is `SECURITY DEFINER` + `SET search_path = public`, owns Vault reads, calls `net.http_post` with `Authorization: Bearer <TASKS_CRON_SERVICE_ROLE_KEY>` + `x-cron-secret: <TASKS_CRON_SECRET>` + `body: {"mode":"cron"}`, and returns the request id. `REVOKE ALL FROM PUBLIC/anon/authenticated` + `GRANT EXECUTE TO postgres` only (cron schedule body runs as the scheduling role). **`public.generate_task_instances(text, date[])`** is `SECURITY DEFINER` + `SET search_path = public`, owns the partial-unique-index `ON CONFLICT (template_id, due_date) WHERE template_id IS NOT NULL DO NOTHING` contract, returns inserted row count via `GET DIAGNOSTICS`. `REVOKE ALL FROM PUBLIC/anon/authenticated` + `GRANT EXECUTE TO service_role` only — the Edge Function is the sole caller. The cron schedule `tasks-cron-daily` runs `SELECT public.invoke_tasks_cron();` at `0 4 * * *` UTC. **No weekly-summary schedule** is registered in Phase B — the Monday 13:00 UTC slot stays locked for Phase F's own creation. **Edge Function auth model** (`supabase/functions/tasks-cron/index.ts`): cron mode requires `Authorization` to equal `env.TASKS_CRON_SERVICE_ROLE_KEY` (NOT auto-injected `SUPABASE_SERVICE_ROLE_KEY` — see deploy lessons below) AND `x-cron-secret` to equal `env.TASKS_CRON_SECRET`, both via constant-time-ish length-then-byte compare; admin mode requires the caller's user JWT in `Authorization` and `rpc('is_admin')` strict `=== true`; mode-mismatch attempts return 401. Optional `body.probe === true` short-circuits to a `tcr-probe-*` audit row + 200 (TEST cleanup is by id-prefix; PROD probe rows are real audit). **Async-delivery audit model** (three layers, no overlap): Layer 1 `cron.job_run_details` (joined to `cron.job` on `jobid` — run_details has no `jobname` column) = "did the schedule fire?"; Layer 2 `net._http_response` = "did the http_post deliver and what status came back?"; Layer 3 `task_cron_runs` = "did the function execute its logic?". A missing Layer 3 row does NOT prove the cron didn't fire — it proves only that the function never ran to completion; recon must walk all three layers. **Recurrence math source-of-truth** lives at `src/lib/tasksRecurrence.js` (pure ESM, zero imports, no Node APIs, no Date mutation leaks); a byte-identical copy at `supabase/functions/_shared/tasksRecurrence.js` is consumed by the Edge Function via Deno's relative ESM resolution; `tests/static/tasks_recurrence_parity.test.js` locks both files against drift via `fs.readFileSync` + CRLF→LF normalize + exact byte match. Anchored monthly/quarterly math: each occurrence computed from the ORIGINAL `first_due_date` plus `n` months, never chained from a previous clamped result; day clamps to `min(F.day, daysInMonth(target_year, target_month))`. `biweekly * recurrence_interval` multiplies the 14-day base. **90-cap = SKIP-and-audit**: when `missing.length > 90` for any active template, the Edge Function pushes `{template_id, horizon_size, capped_at: 90}` to `task_cron_runs.cap_exceeded` and skips that template entirely (no partial generation). **Anon caller of `generate_task_instances`** receives PostgREST `PGRST002 "Could not query the database for the schema cache"` rather than a clean `42501`; same Supabase quirk as `is_admin()`'s anon-deny — security boundary still enforced (anon cannot execute), the test regex on `tests/generate_task_instances_rpc.spec.js` accepts either signal as a valid denial.
- **Tasks Phase B deploy lessons** (worth not relearning the hard way):
  (1) **Function-secret paste via Dashboard UI is whitespace-prone** — pasting `TASKS_CRON_SECRET` into the Edge Function Secrets UI picked up a trailing whitespace (length 97 vs Vault 96), causing the function's `safeEqual` to fail. Mitigation: defensive `envTrim()` at function-load time + use `supabase secrets set` CLI (no whitespace risk) instead of Dashboard paste.
  (2) **TASKS_CRON_SERVICE_ROLE_KEY must be provisioned separately from auto-injected `SUPABASE_SERVICE_ROLE_KEY`** — new Supabase projects auto-inject `SUPABASE_SERVICE_ROLE_KEY` as the new 41-char `sb_secret_*` format, but pg_cron's Vault-backed `Authorization` header sends the legacy 219-char JWT. The function compares the bearer to `env.TASKS_CRON_SERVICE_ROLE_KEY` (provisioned via `supabase secrets set` to the legacy JWT); the unrelated `env.SUPABASE_SERVICE_ROLE_KEY` is still used for the post-auth service-role DB client.
  (3) **`exec_sql` is TEST-ONLY; never install or expose in PROD.** All TEST recon scripts use `exec_sql` to read `pg_constraint` / `pg_extension` / `pg_proc` / `vault.decrypted_secrets` / `cron.job` / `net._http_response` (none exposed via PostgREST). PROD verification is manual SQL Editor only.
  (4) **PostgREST schema-cache quirk**: when a script CREATEs a public table via `exec_sql` then reads it via `.from()`, PostgREST may not see the new table until the cache reloads. Pattern: emit `NOTIFY pgrst, 'reload schema';` + ~1.5s delay before `.from()`. Locked into `ensureScratchTable` in both recon + probe scripts.
  (5) **CLI-canonical layout proven**: `supabase functions deploy tasks-cron --project-ref <ref>` from repo root bundles `supabase/functions/tasks-cron/index.ts` + walks the relative import into `supabase/functions/_shared/tasksRecurrence.js`. The flat `supabase-functions/rapid-processor.ts` layout is incidental legacy; new Edge Functions land at the canonical path.
...
- **Immediate operational lane: Tasks Phase B PROD deploy** (P1-P7 playbook). Phase B is source-shipped on `main` (commit `a44971a`) and TEST-verified end-to-end. **PROD deploy in progress 2026-05-01: P1-P6 GREEN; P7 PENDING.** Status by step:
  - P1 ✓ — Vault provisioned via psql (`vault.create_secret` for `TASKS_CRON_FUNCTION_URL` + `TASKS_CRON_SECRET` minted server-side via `gen_random_bytes(48)` + `TASKS_CRON_SERVICE_ROLE_KEY` = PROD legacy JWT). Verification: all 3 entries length>0.
  - P2 ✓ — `supabase functions deploy tasks-cron --project-ref pzfujbjtayhkdlxiblwe` succeeded; uploaded `index.ts` + `_shared/tasksRecurrence.js`.
  - P3 ✓ — `supabase secrets set --env-file <temp>` for both `TASKS_CRON_SECRET` + `TASKS_CRON_SERVICE_ROLE_KEY`. Defense check: SHA256 of local values matched CLI-reported digests for both (no whitespace drift; the TEST-iteration Dashboard-paste pitfall is avoided here because CLI was used).
  ...
  - P7 ⏳ PENDING — wait for first scheduled cron fire at **2026-05-02 04:00 UTC = 2026-05-01 23:00 CDT**. After fire, audit PROD via psql with `.env.prod.local` PROD_DB_URL:
    ...
    -- (run_details has NO jobname column; join cron.job on jobid)
...
- **Next source build after PROD deploy: Tasks v1 combined-build (Phase C+D+E+F merged).** Plan rev 5 cleared architecture review 2026-05-01 with no architecture blockers. One build, four internal checkpoints C1-C4, functionally gated behind P7 verification. Validation scope: format check, lint, vitest, build, focused Playwright, and RLS/security regression for the touched checkpoint.
  ...
  - **C4 — Weekly summary + new cron schedule + audit table.** New Edge Function `supabase/functions/tasks-summary/index.ts` (canonical CLI layout) + byte-identical helper at `_shared/tasksSummary.js` (parity-locked vs `src/lib/tasksSummary.js` if dual-source needed). Auth model identical to tasks-cron: cron mode = bearer match `TASKS_CRON_SERVICE_ROLE_KEY` + `x-cron-secret` match `TASKS_CRON_SECRET`; admin mode = caller JWT + `rpc('is_admin')`. **Reuses `TASKS_CRON_SECRET` + `TASKS_CRON_SERVICE_ROLE_KEY`**; mints ONE new Vault entry `TASKS_SUMMARY_FUNCTION_URL`. New `mig 042` adds Vault preflight (RAISEs if `TASKS_SUMMARY_FUNCTION_URL` missing/empty), creates `public.invoke_tasks_summary()` SECDEF + cron schedule `tasks-summary-weekly` at `0 13 * * 1` UTC, AND creates a NEW table `public.task_summary_runs` (separate from `task_cron_runs` — do NOT overload `generated_count/skipped_count` with summary semantics; cleaner separation). `task_summary_runs` columns: `id text PK, ran_at, run_mode CHECK ('cron','admin'), recipients_sent int, recipients_skipped int, total_open_instances int, error_message text` + admin-SELECT-only RLS (idempotent DO-block matching mig 037 pattern). `tasks-summary` Edge Function queries open `task_instances` joined to `profiles`, groups by assignee, invokes existing in-repo `supabase-functions/rapid-processor.ts` (legacy flat layout — gets a new `tasks_weekly_summary` payload-type handler branch as part of C4) once per assignee with outstanding tasks, writes one `task_summary_runs` row at end. C4 deploy lane mirrors Phase B P1-P7 (Vault provisioning + CLI function deploys + mig apply + 3-layer audit + post-fire walk).
  - **Common contracts (all 4 checkpoints):** B1 component-level admin guard via `UnauthorizedRedirect`; B2 `allUsers` via existing `useAuth()` context (no new prop threading); B3 Header gates inherit `authState?.role === 'admin'` from existing `Header.jsx:250` block; B4 `src/lib/tasks.js` is pure-only (constants + filters + path converters), side-effectful wrappers in `src/lib/tasksAdminApi.js` + `src/lib/tasksPublicApi.js`. Per-checkpoint Playwright covers: admin access redirect regression (NOT "RLS denies insert"), template CRUD, Run Cron Now idempotency, audit visibility, assignee complete + photo-path validation negatives, RLS isolation, public submit + offline queue + availability filter + assignee eligibility lock + roster-staleness rejection, summary function recipient grouping + auth probes.
...
**Tasks Module v1:** see §8 "Next build" for the active plan (Phase A + Phase B PROD P1-P6 shipped 2026-05-01; P7 pending; combined-build C1-C4 plan rev 5 in §8 "Next build"). Stable locks captured here so future readers don't have to chase the plan packet:

- Assignee = `profiles.id` of a login user only. NOT roster id, NOT team-member display string.
- Submitted-by (public submit only) = master-roster display name from `webform_config.team_roster`, filtered by `webform_config.team_availability.forms['tasks-public'].hiddenIds`. Stored as `task_instances.submitted_by_team_member` text.
- Public submit is one-time only — no recurrence selector on `/webforms/tasks`. Recurring tasks require login + admin Tasks UI.
- Recurrence enum (admin templates): `once|daily|weekly|biweekly|monthly|quarterly`. `recurrence_interval >= 1` integer.
- Generator horizon = current day + 3 days (today+3 ISO date, computed UTC in the Edge Function).
- Weekly summary emails route through the existing in-repo `supabase-functions/rapid-processor.ts` (legacy flat-layout Edge Function) with new `tasks_weekly_summary` payload type added in C4. Recipients = only users with outstanding tasks.
- Cron schedule: daily 04:00 UTC (`tasks-cron-daily`, shipped Phase B mig 039). Weekly summary slot Monday 13:00 UTC (`tasks-summary-weekly`, lands in C4 mig 042 — NOT before).
- NO required-photo behavior anywhere. Optional completion photos LOCKED for C2 only.
