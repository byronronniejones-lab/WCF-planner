...
Priority order for the next session. Items 1–2 shipped + smoked end-of-day 2026-04-27 — left here as a record so the queue starts at #3.

1. ~~**Cattle/sheep processor workflow.**~~ **DONE** (commits `f1adb81` Phase 1 + `448152e` flag-clear fix + `802f393` Phase 2 + `6d70669` Phase 3 + `62e1064` gate loosen). Cattle bidirectional Send-to-Processor sync via `weigh_ins.prior_herd_or_flock` (mig 027); sheep scaffolding (mig 028 + 029, sheep_processing_batches, sheep_transfers, lib, view, modal, audit); sheep weigh-in feature parity (Swap Tag / Missing Tag / Send-to-Processor); SheepHome batches tile. Sheep gate is intentionally looser than cattle's (any draft session, not feeders-only) — see §7.
2. ~~**Pig FCR cache.**~~ **DONE** (commit `a8c7133`). `parent.fcrCached` now stamps on trip add/edit/delete via `computePigBatchFCR` in `src/lib/pig.js`. Adjusted feed ÷ total trip live wt. Cleared via `delete next.fcrCached` when no valid trips remain so transfers fall back to the 3.5 default rather than a stale ratio.
