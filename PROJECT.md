...
**Last consolidated:** 2026-04-21 session 3 (post-polish cutover). **Last session-index update:** 2026-05-01 (Tasks Phase B source-shipped; PROD deploy pending P1-P7).
...
| `task_templates`, `task_instances`, `task_cron_runs` | Tasks Module v1 Phase A (migs 036-038, shipped 2026-05-01) + Phase B (mig 039 + tasks-cron Edge Function, source-shipped 2026-05-01 commit `a44971a`; verified on TEST, PROD deploy pending P1-P7). Phase B drops `requires_photo` from both task tables, extends `recurrence` enum to include `quarterly`, installs `pg_cron`/`pg_net`/`pgcrypto`, and stands up the daily 04:00 UTC generator. Templates are admin-only. Instances are admin-all plus assignee SELECT-own. Cron runs are admin SELECT only and service-role append-only. No UI / public submit / completion RPC yet — those land in Phase C/D/E. |
...
- **Immediate operational lane: Tasks Phase B PROD deploy** (P1-P7 playbook). Phase B is source-shipped on `main` (commit `a44971a`) and TEST-verified end-to-end. **PROD deploy in progress 2026-05-01: P1-P6 GREEN; P7 PENDING.** Status by step:
  ...
  - P7 ⏳ PENDING — wait for first scheduled cron fire at **2026-05-02 04:00 UTC = 2026-05-01 23:00 CDT**. After fire, re-walk: `cron.job_run_details` joined to `cron.job` on `jobid` (run_details has no jobname column) WHERE jobname='tasks-cron-daily' → `net._http_response` → `task_cron_runs`. Confirm same shape as P5: HTTP 200, run_mode='cron', error_message NULL. Missing L3 ≠ "cron didn't fire" — walk all three. Audit-walk SQL block + PROD env loader stays available via `.env.prod.local` (gitignored, machine-local).
- **Next source build after PROD deploy: Tasks v1 combined-build (Phase C+D+E+F merged).** Plan rev 5 approved by Codex 2026-05-01 (no architecture blockers). One build, four internal checkpoints C1-C4, gated behind P7 verification. Each checkpoint has its own pre-merge gate + Ronnie's per-checkpoint "go". Lane approval covers SQL + function deploys inside; commit/push always separate; `exec_sql` in PROD remains forbidden.
