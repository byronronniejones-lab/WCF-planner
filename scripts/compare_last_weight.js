...
(async()=>{
  ...
  const wb=XLSX.readFile('c:/Users/Ronni/OneDrive/Desktop/Cattle upload from Podio/Cattle Tracker - All Cattle Tracker.xlsx',{cellDates:true});
  ...
  const byTag=new Map();
  for(const w of weighIns){if(!w.tag)continue;if(!byTag.has(w.tag))byTag.set(w.tag,[]);byTag.get(w.tag).push(w);}
  ...
  function ourLatest(c){
    ...
    (c.old_tags||[]).forEach(ot=>{ if(ot&&ot.tag&&ot.source!=='import') tags.add(ot.tag); });
    let latest=null;
    for(const t of tags){const l=byTag.get(t);if(l&&l.length){if(!latest||l[0].entered_at>latest.entered_at)latest=l[0];}}
    return latest?parseFloat(latest.weight):null;
  }

  for(const herd of ['mommas','finishers']){
    const cows=cattle.filter(c=>c.herd===herd).sort((a,b)=>(parseFloat(a.tag)||0)-(parseFloat(b.tag)||0));
    const diffs=[];
    let podioSum=0, ourSum=0;
    for(const c of cows){
    ...
    if(diffs.length){
      ...
      diffs.forEach(d=>console.log(`  #${d.tag.padEnd(6)} podio=${d.podio==null?'(blank)':d.podio+' lb'}  ours=${d.ours==null?'(none)':d.ours+' lb'}  diff=${d.diff>0?'+':''}${d.diff.toFixed(1)}`));
    }
  }
})();
