...
const ATTRIBUTIONS = {
  ...
  '2026-04-03': [{subName:'P-26-01A (GILTS)', count:7}, {subName:'P-26-01B (BOARS)', count:3}],
  '2026-04-22': [{subName:'P-26-01A (GILTS)', count:2}, {subName:'P-26-01B (BOARS)', count:2}],
};
...
async function main() {
  ...
  if (gIdx < 0) { console.error('Batch', TARGET_BATCH, 'not found.'); process.exit(2); }
  const g = groups[gIdx];
  ...
  for (let i=0; i<newTrips.length; i++) {
    ...
    if (atts.length === 0) { console.log('    (no attribution change)'); continue; }
    let sum = 0;
    for (const a of atts) {
  ...
  if (w.error) { console.error('write error:', w.error); process.exit(1); }
  console.log('Patched.');
}
