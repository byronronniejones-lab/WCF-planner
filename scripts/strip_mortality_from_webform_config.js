...
(async()=>{
  ...
  if(!rows[0]) { console.log('No full_config row found.'); return; }
  const cfg = rows[0].data;
  const before = JSON.stringify(cfg).length;
  const cattleWf = (cfg.webforms||[]).find(w => w.id === 'cattle-dailys');
  if(!cattleWf) { console.log('No cattle-dailys webform in config — nothing to do.'); return; }
  const hadMort = (cattleWf.sections||[]).some(s => s.id === 's-mortality');
  if(!hadMort) { console.log('cattle-dailys already has no s-mortality section.'); return; }
  ...
  if(!process.argv.includes('--commit')) { console.log('(Preview only. Rerun with --commit to apply.)'); return; }
