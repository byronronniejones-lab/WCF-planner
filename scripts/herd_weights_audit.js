...
(async()=>{
  ...
  Object.entries(herdCounts).sort().forEach(([h,n]) => console.log(`  ${h.padEnd(16)} ${n}`));
  console.log('\nCattle with NO weigh-in, by herd:');
  Object.entries(herdMissingWeight).sort().forEach(([h,n]) => console.log(`  ${h.padEnd(16)} ${n}`));
  console.log('\nTotal live weight per herd (from latest weigh-in):');
  let grand = 0;
  Object.entries(herdTotals).sort().forEach(([h,w]) => { console.log(`  ${h.padEnd(16)} ${Math.round(w).toLocaleString()} lb`); grand += w; });
  ...
  active.forEach(c => console.log(`  #${c.tag||'(no tag)'}  herd=${c.herd}  old_tags=${JSON.stringify((c.old_tags||[]).map(t=>t.tag))}`));
})();
