...
function nameMatches(d, names) {
  const lbl = String(d.batch_label||'').trim().toLowerCase();
  const bid = String(d.batch_id||'').trim().toLowerCase();
...
async function main() {
  ...
  if (f.error) { console.error('feeders read error:', f.error); process.exit(1); }
  const groups = (f.data && Array.isArray(f.data.data)) ? f.data.data : [];
  const g = groups.find(x => x.batchName === TARGET_BATCH);
  if (!g) { console.error('Batch', TARGET_BATCH, 'not found.'); process.exit(2); }
  ...
  const transfers = breeders.filter(br => br && br.transferredFromBatch && br.transferredFromBatch.batchName === TARGET_BATCH);

  // 3) pig_dailys (paginated)
  const pdAll = [];
  for (let from=0;;from+=1000) {
    const {data, error} = await sb.from('pig_dailys').select('*').range(from, from+999);
    if (error) { console.error('pig_dailys read error:', error); process.exit(1); }
  ...
  if (tripIds.length > 0) {
    ...
    for (let i=0;i<tripIds.length;i+=200) {
      ...
      const {data, error} = await sb.from('weigh_ins').select('id,session_id,sent_to_trip_id,sent_to_group_id,weight,note,transferred_to_breeding,transfer_breeder_id,feed_allocation_lbs').in('sent_to_trip_id', slice);
      if (error) { console.error('weigh_ins read error:', error); process.exit(1); }
  ...
  const sessMap = {};
  if (sessIds.length > 0) {
    const {data: sessions, error} = await sb.from('weigh_in_sessions').select('id,batch_id,date,species').in('id', sessIds);
    if (error) { console.error('weigh_in_sessions read error:', error); process.exit(1); }
  ...
  for (const tr of (g.processingTrips||[])) {
    ...
    const tripLive = (tr.liveWeights||'').split(/[\s,]+/).map(parseFloat).filter(v=>!isNaN(v)&&v>0).reduce((a,b)=>a+b,0);
  ...
  const subTripAggByName = {__unattributed__: subTripAgg.__unattributed__ || {pigCount:0,liveSum:0,weighinCount:0}};
  ...
  for (const tr of (g.processingTrips||[])) {
    ...
    console.log('  ['+tr.date+']', 'pigCount='+(tr.pigCount||0), 'live='+Math.round(tripLive), 'hang='+(tr.hangingWeight||0), 'sources:', JSON.stringify(sources.counts));
  }
  ...
  for (const t of transfers) {
    ...
    trBySub[sub].items.push({tag:t.tag, sex:t.sex, feedAllocLb:t.transferredFromBatch.feedAllocationLbs, fcrUsed:t.transferredFromBatch.fcrUsed, weighin:t.transferredFromBatch.sourceWeighInId, date:t.transferredFromBatch.transferDate});
  }
  for (const [sub, agg] of Object.entries(trBySub)) {
    console.log('  '+sub+': '+agg.count+' transferred, '+Math.round(agg.feedSum)+' lbs feed credit');
    for (const it of agg.items) console.log('    -', it.tag||'(no tag)', it.sex||'?', '@', it.weighin||'?', '·', Math.round(n(it.feedAllocLb))+'lb / FCR '+(it.fcrUsed||'?'));
  }
  ...
  for (const s of subs) {
    ...
    const latestDaily = [...sd.rows].sort((a,b)=>String(b.date).localeCompare(String(a.date))||String(b.submitted_at||'').localeCompare(String(a.submitted_at||'')))[0]||null;
    ...
    console.log('  latest daily pig_count    :', latestDaily ? `${latestDaily.pig_count} on ${latestDaily.date}` : '(no dailys)');
  ...
  console.log('  parent totalFeed (raw − allocOut):', Math.round(sumSubFeedRaw + n(g.legacyFeedLbs) - n(g.feedAllocatedToTransfers)));
  console.log('  diff (alloc-on-parent vs sum-of-sub-credits):', Math.round(n(g.feedAllocatedToTransfers) - sumSubTransferFeed));
  console.log('');
  ...
  if (subTripAggByName.__unattributed__ && subTripAggByName.__unattributed__.pigCount > 0) {
    console.log('  ⚠ unattributed trip pigs      :', subTripAggByName.__unattributed__.pigCount, '(no weigh_in source — pre-Send-to-Trip)');
  }
