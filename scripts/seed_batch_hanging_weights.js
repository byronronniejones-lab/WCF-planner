...
(async () => {
  ...
  const cattle = await fetchAll('cattle?select=id,tag,processing_batch_id,hanging_weight&processing_batch_id=not.is.null&');
  const sums = new Map();
  ...
  plans.forEach(p => console.log(`  ${p.name.padEnd(22)} ${p.oldTotal} \u2192 ${Math.round(p.newTotal*10)/10} lb`));
  console.log(`\nBatches to update: ${plans.length}`);

  if (!process.argv.includes('--commit')) { console.log('\n(Preview only. Rerun with --commit to apply.)'); return; }
