...
async function main() {
  ...
  for (let from=0;;from+=1000) {
    const {data, error} = await sb.from('cattle')
  ...
  for (let from=0;;from+=1000) {
    ...
    const {data, error} = await sb.from('cattle_transfers')
  ...
  for (const c of cowsWithBatch) {
    const m = transfers.find(t => t.cattle_id === c.id && t.reason === 'processing_batch' && t.reference_id === c.processing_batch_id);
    if (m) matched++; else missing.push(c);
  }
  console.log('');
