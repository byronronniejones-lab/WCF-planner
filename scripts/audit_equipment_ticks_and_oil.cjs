...
(async () => {
  ...
  if (!hijet) {
    ...
    const {data: rows} = await sb.from('equipment_fuelings')
      .select('id,date,km_reading,hours_reading,team_member,every_fillup_check,service_intervals_completed,photos,podio_item_id,podio_source_app,comments')
    ...
    if (!rows || rows.length === 0) {
      ...
      for (const r of rows) {
        ...
        console.log(`    team=${r.team_member}  reading=${r.km_reading||r.hours_reading}  comments=${(r.comments||'').slice(0,60)}`);
        console.log(`    every_fillup_check: ${Array.isArray(r.every_fillup_check)?r.every_fillup_check.length:'(not array)'} items`);
        if (Array.isArray(r.every_fillup_check)) {
          for (const c of r.every_fillup_check) console.log(`      · ${c.label}`);
        }
        console.log(`    service_intervals_completed: ${Array.isArray(r.service_intervals_completed)?r.service_intervals_completed.length:'(not array)'} entries`);
        if (Array.isArray(r.service_intervals_completed)) {
          for (const c of r.service_intervals_completed) {
            console.log(`      · ${c.interval}${c.kind.charAt(0)} — ${Array.isArray(c.items_completed)?c.items_completed.length:0}/${c.total_tasks||'?'} ticked`);
          }
        }
  ...
  for (const [src, s] of Object.entries(bySource).sort((a,b)=>b[1].total-a[1].total)) {
    console.log(`    ${src.padEnd(45)}  total=${String(s.total).padStart(4)}  fillup=${String(s.withFillup).padStart(4)}  svc=${String(s.withSvc).padStart(4)}`);
  }

  // B) Audit every_fillup_items for Check Oil across active equipment
  console.log('\n═══ B. every_fillup_items Check Oil audit ═══');
  const {data: eqs} = await sb.from('equipment').select('slug,name,category,every_fillup_items').eq('status','active').order('category').order('name');
  const missing = [];
  for (const eq of (eqs||[])) {
    ...
    console.log(`  ${marker} ${eq.category.padEnd(10)} ${eq.slug.padEnd(18)} ${eq.name.padEnd(30)} items=${items.length}${note}`);
