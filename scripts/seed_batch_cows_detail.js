...
async function main() {
  ...
  const [batches, cattle, cattleSessions, allWIs] = await Promise.all([
    fetchAll('cattle_processing_batches', 'select=id,name,actual_process_date,cows_detail,total_live_weight,total_hanging_weight'),
  ...
  console.log(`  ${batches.length} batches, ${cattle.length} cattle, ${cattleSessions.length} cattle sessions, ${allWIs.length} total weigh_ins`);
  ...
  function latestWeightFor(cow, cutoffDate) {
    ...
    const candidates = cattleWIs
      ...
      .sort((a,b) => (b._date||'').localeCompare(a._date||'') || (b.entered_at||'').localeCompare(a.entered_at||''));
  ...
  for (const p of plan) {
    ...
    console.log(`  ${p.batch.name.padEnd(24)}  ${p.cows_detail.length} cows  live=${liveStr.padStart(10)}  hanging=${hangStr.padStart(10)}`);
  ...
  for (const p of plan) {
    ...
    if (pr.ok) { ok++; } else { fail++; console.error(`  FAIL: ${p.batch.name}: ${pr.status} ${await pr.text()}`); }
  }
