...
dupeGroups.slice(0,6).forEach(g => {
  const r = g[0];
  const d = r['Date'] instanceof Date ? r['Date'].toISOString().slice(0,10) : '';
  console.log(`  ${d} | ${r['Team member']} | ${r['Cattle Group']} | DM=${r['DM Given  ']} | voltage=${r['Fence Voltage - KV']} | cost=${r['Hay & Pellets cost']} | issues="${String(r['Issues / Mortalities / Comments']||'').slice(0,40)}"  (${g.length} copies)`);
});
