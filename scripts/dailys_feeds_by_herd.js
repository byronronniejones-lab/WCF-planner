...
const byHerd = {};
for (const r of rows) {
  const h = r['Cattle Group']; if (!byHerd[h]) byHerd[h] = new Set();
  ['Hay Type #1','Hay Type #2','Hay Type #3'].forEach(c => {
    const v = r[c]; if (v && String(v).trim()) byHerd[h].add(String(v).trim());
  });
...
for (const r of rows) {
  for (const i of [1,2,3]) {
    ...
    s.samples++; s.sumP += p; s.sumN += n; s.sumDM += dm; s.sumBales += bales;
  }
}
