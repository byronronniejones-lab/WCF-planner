...
function analyseFeedInputs(rows) {
  ...
  for (const r of rows) {
    ...
    if (c > 0) {
      pellets.citrus.sumQty += c;
      pellets.citrus.sumP += num(r['Lbs Protein - Citrus Pellets']);
      pellets.citrus.sumN += num(r['Lbs NFC - Citrus Pellets']);
    }
    const a = num(r['Lbs of Alfalfa Pellets']);
    if (a > 0) {
      pellets.alfalfa.sumQty += a;
      pellets.alfalfa.sumP += num(r['Lbs Protein - Alfalfa Pellets']);
      pellets.alfalfa.sumN += num(r['Lbs NFC - Alfalfa Pellets']);
    }
  }
  const pelletInputs = [];
  if (pellets.citrus.sumQty > 0)
    pelletInputs.push({
...
function buildPlan() {
  ...
  const feedIdMap = {};
  feedInputs.forEach((f) => {
    feedIdMap[f.category + '|' + f.name] = f.id;
  });
  ...
  for (const r of raw) {
    ...
    const sKey = [
      ...
      water_checked,
      num(r['DM Given  ']),
      num(r['Hay & Pellets cost'] || 0),
      JSON.stringify(feeds),
    ].join('|');
    if (dupeKeys.has(sKey)) {
      skipped.dupes++;
      continue;
    }
    dupeKeys.add(sKey);
...
function printPreview(plan) {
  ...
  console.log(
    `Skipped: blank_date=${plan.skipped.blankDate}  other_group=${plan.skipped.other}  strict_dupes=${plan.skipped.dupes}`,
  );
  ...
  Object.entries(byHerd)
    .sort()
    .forEach(([h, n]) => console.log(`  ${h.padEnd(12)} ${n}`));

  console.log(`\nFeed inputs to upsert: ${plan.feedInputs.length}`);
  plan.feedInputs.forEach((f) => {
    console.log(
      `  ${f.category.padEnd(7)} ${f.name.padEnd(28)} P=${f.protein_pct}%  NFC=${f.nfc_pct}%  unit=${f.unit}  unit_wt=${f.unit_weight_lbs}`,
    );
  });
  ...
  if (sample) {
    console.log('\nSample daily row:');
    console.log(
      `  ${sample.date} ${sample.team_member} ${sample.herd} voltage=${sample.fence_voltage} water=${sample.water_checked} issues="${sample.issues}"`,
    );
    console.log(`  feeds: ${sample.feeds.length} entries`);
    sample.feeds.forEach((f) =>
      console.log(
        `    ${f.qty} ${f.unit} ${f.feed_name} (${f.category})  DM/asfed=${f.lbs_as_fed}  P=${f.nutrition_snapshot.protein_pct}%  NFC=${f.nutrition_snapshot.nfc_pct}%`,
      ),
    );
  }
