...
(async () => {
  ...
  for (const c of mommas) {
    ...
    let all = [];
    for (const t of tags) {
      if (byTag.has(t)) all = all.concat(byTag.get(t));
    }
    ...
    if (isRcv && hasScaleSameDate) {
      ...
      suspects.push({
        tag: c.tag,
        picked: latest.weight,
        scale: scale.weight,
        date: (latest.entered_at || '').slice(0, 10),
      });
    }
  }
  ...
  suspects.forEach((s) =>
    console.log(
      `  #${s.tag}  picked=${s.picked} lb  scale=${s.scale} lb  date=${s.date}  (diff ${s.picked - s.scale})`,
    ),
  );
