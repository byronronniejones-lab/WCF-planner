...
const OPERATOR_NOTES = {
  ...
  'honda-atv-1':
    '#1 - 2024 HONDA ATV FUELING CHECKLIST. 3.88 GALLON FUEL TANK. 5 BARS ON FUEL GAUGE. THE FIRST 4 BARS ARE .5 GALLON EACH. WHEN YOU REACH THE LAST BAR YOU HAVE 1.8 GALLONS LEFT OR ALMOST 1/2 TANK. WHEN "LO FUEL" SHOWS ON SCREEN YOU HAVE 1.2 GAL LEFT.',
  'honda-atv-2':
    '#2 - 2024 HONDA ATV FUELING CHECKLIST. 3.88 GALLON FUEL TANK. 5 BARS ON FUEL GAUGE. THE FIRST 4 BARS ARE .5 GALLON EACH. WHEN YOU REACH THE LAST BAR YOU HAVE 1.8 GALLONS LEFT OR ALMOST 1/2 TANK. WHEN "LO FUEL" SHOWS ON SCREEN YOU HAVE 1.2 GAL LEFT.',
  'honda-atv-3':
    '#3 - 2024 HONDA ATV FUELING CHECKLIST. 3.88 GALLON FUEL TANK. 5 BARS ON FUEL GAUGE. THE FIRST 4 BARS ARE .5 GALLON EACH. WHEN YOU REACH THE LAST BAR YOU HAVE 1.8 GALLONS LEFT OR ALMOST 1/2 TANK. WHEN "LO FUEL" SHOWS ON SCREEN YOU HAVE 1.2 GAL LEFT.',
  'honda-atv-4':
    '#4 - 2024 HONDA ATV FUELING CHECKLIST. 3.88 GALLON FUEL TANK. 5 BARS ON FUEL GAUGE. THE FIRST 4 BARS ARE .5 GALLON EACH. WHEN YOU REACH THE LAST BAR YOU HAVE 1.8 GALLONS LEFT OR ALMOST 1/2 TANK. WHEN "LO FUEL" SHOWS ON SCREEN YOU HAVE 1.2 GAL LEFT.',
...
(async () => {
  const {data: eqs} = await sb
    .from('equipment')
    .select('id,slug,name,operator_notes')
    .in('slug', Object.keys(OPERATOR_NOTES));
  ...
  for (const [slug, text] of Object.entries(OPERATOR_NOTES)) {
    ...
    if (!eq) {
      console.log(`  ✗ ${slug} not in equipment table`);
      continue;
    }
    const current = (eq.operator_notes || '').trim();
    const next = text.trim();
    if (current === next) {
      console.log(`  · ${slug}: already matches`);
      continue;
    }
