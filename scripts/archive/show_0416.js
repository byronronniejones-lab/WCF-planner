...
(async () => {
  const r = await fetch(
    `${URL}/rest/v1/weigh_in_sessions?select=id,date,notes,team_member&species=eq.cattle&date=eq.2026-04-16`,
    {headers: H},
  );
  ...
  for (const s of sess) {
    const w = await fetch(
      `${URL}/rest/v1/weigh_ins?session_id=eq.${encodeURIComponent(s.id)}&select=id,tag,weight,note`,
      {headers: H},
    );
