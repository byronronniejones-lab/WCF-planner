...
function fieldCategoryValues(item, external_id) {
  ...
  return (f.values || [])
    .filter((v) => v?.value && (activeOptionIds.size === 0 || activeOptionIds.has(v.value.id)))
    .map((v) => v.value.text)
    .filter(Boolean);
}
...
function deterministicFuelingId(prefix, itemId) {
  return `fuel-${prefix}-${itemId}`;
}
...
(async () => {
    ...
    for (const item of items) {
      ...
      if (item.fields) {
        for (const f of item.fields) {
          ...
          const ticked = (f.values || [])
            .filter((v) => v?.value && (activeOpts.size === 0 || activeOpts.has(v.value.id)))
            .map((v) => v.value.text)
            .filter(Boolean);
          if (ticked.length === 0) continue;
          const total = (f.config?.settings?.options || []).filter((o) => o.status !== 'deleted').length;
          const itemsCompleted = ticked.map((t) =>
            t
              .toLowerCase()
              .replace(/[^a-z0-9]+/g, '-')
              .replace(/^-|-$/g, '')
          ...
          if (everyUse) {
            ...
            if (firstEvery) values = [Number(firstEvery[2])];
              ...
              const nums = [...upLabel.matchAll(/(\d{1,3}(?:,\d{3})+|\d+)/g)]
      ...
      const gallons =
        fieldNumValue(item, 'gallons') ||
        fieldNumValue(item, 'gallons-of-diesel') ||
        fieldNumValue(item, 'gallons-of-gasoline');
      const row = {
  ...
  for (const x of toInsert.slice(0, 20))
    console.log(
      `  · ${x.slug.padEnd(16)} date=${x.row.date} team=${x.row.team_member} hrs=${x.row.hours_reading || x.row.km_reading} gal=${x.row.gallons || '—'} podio=${x.item_id} related=${x.related.join(',')}`,
    );
  ...
  if (!COMMIT) {
    console.log('\nPreview only — rerun with --commit to insert.');
    return;
  }
