...
(async () => {
  ...
  const checklistApps = (summary.apps || []).filter((a) => a.slug !== 'fuel-log' && a.slug !== 'equipment-maintenance');
  ...
  for (const app of checklistApps) {
    ...
    for (const item of items) {
      ...
      for (const fid of relatedLogIds) {
        if (fuelByPodioId.has(fid)) {
          targetRow = fuelByPodioId.get(fid);
          break;
        }
      }
  ...
  for (const u of updates) {
    const {error} = await sb
      .from('equipment_fuelings')
      .update({every_fillup_check: u.every_fillup_check})
      .eq('id', u.id);
    if (error) {
