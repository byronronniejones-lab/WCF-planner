...
(async () => {
  ...
  for (let from = 0; ; from += 1000) {
    const {data, error} = await sb
      ...
      .select(
        'id,equipment_id,date,gallons,team_member,every_fillup_check,service_intervals_completed,podio_source_app,comments',
      )
      .range(from, from + 999);
    if (error) {
      console.error(error);
  ...
  console.log(
    `Rows with no checklist content (to DELETE): ${toDelete.length} (${Math.round(totalGal).toLocaleString()} gal)`,
  );
