...
(async () => {
  ...
  if (!eq) { console.log('No ventrac equipment row.'); process.exit(0); }
  ...
  for (const a of planner) {
    console.log(`  ${a.name} · ${a.kind} · ${a.hours_or_km === 0 ? 'Every Use' : 'Every '+a.hours_or_km+a.kind.charAt(0)} — ${(a.tasks||[]).length} tasks${a.help_text?' [has help]':''}`);
  }

  // ── Source of truth 1: Podio WEBFORM snapshot (Tough Cut + AERO-Vator only, since Landscape Rake isn't published) ──
  const snap = JSON.parse(fs.readFileSync(path.join(__dirname, 'podio_equipment_dump', '_webforms_snapshot_2026-04-24.json'), 'utf8'));
  const webformAtts = (snap.forms.ventrac.fields || []).filter(f => f.attachment).map(f => {
  ...
  for (const a of webformAtts) {
    console.log(`  ${a.name} · ${a.kind} · ${a.hours_or_km === 0 ? 'Every Use' : 'Every '+a.hours_or_km+a.kind.charAt(0)} — ${a.options.length} tasks${a.help?' [has help]':''}`);
  }

  // ── Source of truth 2: Podio DUMP (includes Landscape Rake even though unpublished) ──
  const dumpConfig = JSON.parse(fs.readFileSync(path.join(__dirname, 'podio_equipment_dump', '30089562.ventrac-fueling-checklists.config.json'), 'utf8'));
  const dumpAtts = [];
  for (const f of (dumpConfig.fields || [])) {
    ...
    const options = ((f.config && f.config.settings && f.config.settings.options) || [])
      .filter(o => o.status !== 'deleted').map(o => o.text);
  ...
  for (const dump of dumpAtts) {
    ...
    const pl = planner.find(a => norm(a.name) === norm(dump.name) && a.kind === dump.kind && a.hours_or_km === dump.hours_or_km);
    if (!pl) { diffs.push({kind:'missing_from_planner', dump}); continue; }
    ...
    if (pHelp !== dHelp) diffs.push({kind:'help_differ', dump, pl, podio_help: dump.help, planner_help: pl.help_text || ''});
  }
  for (const pl of planner) {
    const match = dumpAtts.find(a => norm(a.name) === norm(pl.name) && a.kind === pl.kind && a.hours_or_km === pl.hours_or_km);
  ...
  if (diffs.length === 0) {
    ...
    for (const d of diffs) {
      if (d.kind === 'missing_from_planner') {
        ...
        if (d.missing.length) { console.log('      MISSING from planner:'); for (const o of d.missing) console.log('        - '+o); }
        if (d.extra.length)   { console.log('      EXTRA in planner:');     for (const o of d.extra)   console.log('        - '+o); }
