...
console.log('Date range:',
  otherRows.map(r => r.Date).filter(Boolean).map(d => d instanceof Date ? d.toISOString().slice(0,10) : d).sort());
console.log('\nFirst 10 OTHER rows:');
...
trackerRows.filter(r => r['Weight History w/ Date']).slice(0, 5).forEach((r, i) => {
  console.log(`  Tag ${r['Tag #']}: ${String(r['Weight History w/ Date']).slice(0, 200)}`);
});
console.log('\nLambs column — first 5 non-empty samples:');
trackerRows.filter(r => r.Lambs).slice(0, 5).forEach(r => {
