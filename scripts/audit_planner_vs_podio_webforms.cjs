...
const snapshot = JSON.parse(fs.readFileSync(path.join(__dirname, 'podio_equipment_dump', '_webforms_snapshot_2026-04-24.json'), 'utf8'));
...
(async () => {
  ...
  for (const slug of Object.keys(snapshot.forms)) {
    ...
    if (!eq) { console.log(`\n═══ ${slug} — NOT FOUND in planner (status!=active?) ═══`); continue; }
    ...
    if (fillupField) {
      ...
      if (podioHelp !== planHelp) {
        diffs.push({kind:'every_fillup_help', podio: fillupField.help || '(none)', planner: eq.every_fillup_help || '(none)'});
      }
    }
    ...
    if (gallonsField) {
      ...
      if (podioHelp !== planHelp) {
        diffs.push({kind:'fuel_gallons_help', podio: gallonsField.help || '(none)', planner: eq.fuel_gallons_help || '(none)'});
      }
    }
    ...
    for (const p of podioIvs) {
      ...
      if (!pl) {
        diffs.push({kind:'interval_missing', interval: `${p.value}${p.kind}`, podio_label: p.label, podio_tasks: p.options});
        continue;
      }
      // Label match
      ...
      if (missingFromPlanner.length || extraInPlanner.length) {
        diffs.push({kind:'interval_tasks', interval: `${p.value}${p.kind}`, missing: missingFromPlanner, extra: extraInPlanner});
      }
      // Help text
      const podioHelp = norm(p.help);
      const planHelp = norm(pl.help_text || '');
      if (podioHelp !== planHelp) {
        diffs.push({kind:'interval_help', interval: `${p.value}${p.kind}`, podio: p.help || '(none)', planner: pl.help_text || '(none)'});
      }
    }
    ...
    for (const pl of planIvs) {
      ...
      if (!match) {
        diffs.push({kind:'interval_extra_in_planner', interval: `${pl.hours_or_km}${pl.kind}`, planner_label: pl.label});
      }
    }
    ...
    for (const d of diffs) {
      if (d.kind === 'operator_notes') {
        ...
        if (d.missing.length) console.log(`  [every_fillup_items] MISSING in planner: ${d.missing.map(x=>`"${x}"`).join(', ')}`);
        if (d.extra.length)   console.log(`  [every_fillup_items] EXTRA in planner (not in Podio): ${d.extra.map(x=>`"${x}"`).join(', ')}`);
        ...
        if (d.missing.length) console.log(`  [interval ${d.interval}] MISSING task(s) in planner: ${d.missing.map(x=>`"${x}"`).join(', ')}`);
        if (d.extra.length)   console.log(`  [interval ${d.interval}] EXTRA task(s) in planner (not on Podio): ${d.extra.map(x=>`"${x}"`).join(', ')}`);
