...
const HAY_SPECS = [
  ...
  { name: 'CRABGRASS',                  dm: 769.50, moisture: 47.5,  protein: 15.0,  nfc: 8.0  },
];
const PELLET_SPECS = [
...
async function fetchCurrent() {
  const res = await fetch(`${URL}/rest/v1/cattle_feed_inputs?select=id,name,unit,unit_weight_lbs,moisture_pct,protein_pct,nfc_pct`, { headers: H });
...
(async () => {
  ...
  for (const p of PELLET_SPECS) {
    ...
    if (!found) { console.warn(`  (!) ${p.name} not in DB — skipped`); continue; }
    patches.push({
      id: found.id, name: p.name, unit: 'lb',
      set: {
        unit_weight_lbs: 1,
  ...
  for (const p of patches) {
    ...
    console.log(`    current: bale=${cur.unit_weight_lbs} · moist=${cur.moisture_pct} · P=${cur.protein_pct}% · NFC=${cur.nfc_pct}%`);
