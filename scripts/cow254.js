...
(async()=>{
  ...
  const w=await fetch(`${URL}/rest/v1/weigh_ins?tag=eq.254&order=entered_at.desc&select=weight,entered_at,note,session_id`,{headers:H});
  console.log('\nWeigh-ins under tag 254 (our DB):');
  (await w.json()).forEach(r=>console.log(`  ${(r.entered_at||'').slice(0,10)}  ${r.weight} lb  ${r.note||''}  sid=${r.session_id}`));
  ...
  hits.sort((a,b)=>(b['Date']||0)-(a['Date']||0)).forEach(r=>{
