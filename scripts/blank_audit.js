...
const wb = XLSX.readFile('c:/Users/Ronni/OneDrive/Desktop/Cattle upload from Podio/Cattle Tracker - All Cattle Tracker.xlsx');
...
Object.entries(colCounts).sort((a,b) => b[1] - a[1]).forEach(([k,c]) => console.log(`  ${k}: ${c}`));

console.log('\nFirst 3 blank-tag rows with data (if any):');
blanks.filter(r => Object.entries(r).some(([k,v]) => k !== tagKey && v != null && String(v).trim() !== ''))
  .slice(0, 3).forEach((r, i) => {
    console.log(`--- row ${i+1} ---`);
    Object.entries(r).forEach(([k,v]) => { if (v != null && String(v).trim() !== '') console.log(`  ${k}: ${String(v).slice(0, 80)}`); });
  });
