...
(async()=>{
  ...
  const sessions = await fetchAll(`weigh_in_sessions?select=id,date&id=in.(${sessionIds.map(s=>`"${s}"`).join(',')})&`);
  ...
  if(!process.argv.includes('--commit')){
    ...
    weighIns.slice(0,10).forEach(w => {
      ...
      console.log(`  ${w.id}  session_date=${d}  current_entered_at=${(w.entered_at||'').slice(0,19)}  -> ${d}T12:00:00Z`);
    });
