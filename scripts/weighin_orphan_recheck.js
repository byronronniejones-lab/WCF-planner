...
for (const r of rows) {
  ...
  if (t && c && t !== c) {
    if (count < 8) {
      console.log(
        `  Tag #=${t}  Cow=${c}  Date=${r['Date'] ? r['Date'].toISOString().slice(0, 10) : '?'}  Weight=${r['Weight']}`,
      );
      count++;
    }
  }
...
(async () => {
  ...
  let resolvedByCurrentTag = 0,
    resolvedByCowField = 0,
    resolvedByOldTag = 0,
    unresolved = 0;
  ...
  for (const r of rows) {
    ...
    const c = String(r['Cow'] || '').trim();
    if (tagToCows.has(t)) {
      resolvedByCurrentTag++;
      continue;
    }
    if (c && c !== t && tagToCows.has(c)) {
      resolvedByCowField++;
      continue;
    }
    if (oldTagToCows.has(t)) {
      resolvedByOldTag++;
      continue;
    }
    if (c && c !== t && oldTagToCows.has(c)) {
      resolvedByOldTag++;
      continue;
    }
    unresolved++;
