  ...
  for (const r of dailysRows) {
    ...
    if (!flock) {
      skippedDailys.push({
        ...
        reason: `unknown group "${r.group_raw}"`,
        team: r.team_member,
        date: r.date,
      });
      continue;
...
function printPreview(plan) {
  ...
  plan.lambing
    .slice(0, 5)
    .forEach((l) => console.log(`  ${l.id.padEnd(14)} dam=${l.dam_tag} → lamb=${l.lamb_tag} (${l.lambing_date})`));
  ...
  plan.weighIns
    .slice(0, 5)
    .forEach((w) => console.log(`  tag=${w.tag.padEnd(6)} weight=${w.weight} (session ${w.session_id})`));
  ...
  Object.entries(byDailyFlock)
    .sort()
    .forEach(([f, n]) => console.log(`  ${f.padEnd(12)} ${n}`));

  if (plan.skippedDailys.length > 0) {
    console.log(`\nSkipped dailys rows: ${plan.skippedDailys.length}`);
    plan.skippedDailys.forEach((s) =>
      console.log(
        `  xlsx row ${s.rowindex}: ${s.reason} (team=${s.team || '-'}, date=${s.date || '-'}, group=${s.group || '-'})`,
      ),
    );
  }
