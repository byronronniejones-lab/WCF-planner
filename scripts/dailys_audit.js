...
// Group counts
const byGroup = {};
for (const r of rows) { const g = r['Cattle Group']; byGroup[g] = (byGroup[g]||0) + 1; }
...
others.slice(0,5).forEach((r,i) => {
  ...
  console.log(`  ${d}  team=${r['Team member']}  DM=${r['DM Given  ']}  voltage=${r['Fence Voltage - KV']}  issues="${String(r['Issues / Mortalities / Comments']||'').slice(0,60)}"`);
});

// Blank-date rows
const blankDate = rows.filter(r => !r['Date']);
console.log(`\nBlank-date rows: ${blankDate.length}`);
blankDate.slice(0,3).forEach(r => console.log('  ', JSON.stringify(Object.fromEntries(Object.entries(r).filter(([,v])=>v!=null&&String(v).trim()!==''))).slice(0,200)));

// Date range
const dates = rows.map(r => r['Date']).filter(x => x instanceof Date).sort((a,b) => a-b);
...
// "None"-only issues
const noneCount = rows.filter(r => String(r['Issues / Mortalities / Comments']||'').trim().toLowerCase() === 'none').length;
