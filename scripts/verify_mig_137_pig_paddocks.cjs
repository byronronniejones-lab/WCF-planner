...
if (ENV === 'prod') {
  ...
  if (url && url.includes(PROD_REF)) { console.error('test env points at PROD; aborting'); process.exit(2); }
} else {
  console.error("env must be 'prod' or 'test'"); process.exit(2);
}
if (!url || !key) { console.error(`missing url/key for ${ENV}`); process.exit(2); }
...
let pass = 0, fail = 0;
const ok = (l, c, d) => { c ? pass++ : fail++; console.log(`  ${c ? 'PASS' : 'FAIL'} ${l}${d ? ' — ' + d : ''}`); };

(async () => {
  console.log(`VERIFY mig 137 against ${ENV.toUpperCase()} (${url})`);
  const {data: areas, error} = await svc.from('land_areas')
    .select('id,name,kind,permanence,designation,status,review_status,geometry_status,computed_acres,parent_id,deleted_at')
    .is('deleted_at', null);
  if (error) { console.error(error.message); process.exit(1); }

  const parents = areas.filter((a) => PARENTS.includes(a.name) && a.kind === 'pasture');
  ok('4 parent pig pastures present (kind=pasture, active)',
    parents.length === 4 && parents.every((p) => p.status === 'active'), `${parents.length}`);
  ...
  ok('all paddocks designation=feeder_pig, active, reviewed',
    pads.every((p) => p.designation === 'feeder_pig' && p.status === 'active' && p.review_status === 'reviewed'));
  ok('all paddocks geometry_status=valid', pads.every((p) => p.geometry_status === 'valid'));
  ok('all paddock ids deterministic la-pigpad-*', pads.every((p) => /^la-pigpad-[a-d]\d+$/.test(p.id)));
  ...
  for (const par of parents) {
    ...
    ok(`${par.name}: 10 children & acres≈parent`,
  ...
  if (ids.length) {
    const {data: gvs} = await svc.from('land_area_geometry_versions').select('land_area_id,version_number').in('land_area_id', ids);
    const byArea = {};
    ...
    ok('every paddock has exactly 1 geometry version (v1)', ids.length === Object.keys(byArea).length && bad.length === 0);
  }
