...
const wb = XLSX.readFile("c:/Users/Ronni/OneDrive/Desktop/Cattle upload from Podio/Cattle Daily's - All Cattle Daily's.xlsx", { cellDates: true });
...
Object.entries(fill).sort((a,b) => b[1]-a[1]).forEach(([k,n]) => console.log(`  ${n.toString().padStart(4)}  ${k}`));
...
for (const col of Object.keys(rows[0]||{})) {
  ...
  if (vals.size > 0 && vals.size <= 20) {
    ...
    [...vals].sort().slice(0, 20).forEach(v => console.log(`    - ${v}`));
  }
}
