// Applier for migration 049 (pig_slug + pig_session_metrics RPC) against
// the test Supabase project. Mirrors apply_test_mig_048.cjs:
// reads .env.test + .env.test.local, validates we're not pointed at prod,
// strips outer BEGIN/COMMIT (exec_sql cannot EXECUTE transaction boundaries
// from inside its function body), runs the migration via exec_sql.
//
// Idempotent — every CREATE uses CREATE OR REPLACE. Re-running is a no-op
// against an already-applied test DB.
//
// Usage: node scripts/apply_test_mig_049.cjs
...
(async () => {
  ...
  const file = '049_pig_session_metrics_rpc.sql';
