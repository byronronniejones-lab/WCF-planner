...
if (!path) { console.error('usage: node inspect_sheep_tracker.js <path>'); process.exit(1); }
...
Object.entries(fill).sort((a,b) => b[1]-a[1]).forEach(([k,n]) => console.log(`  ${String(n).padStart(4)}  ${k}`));
...
for (const col of Object.keys(rows[0] || {})) {
  ...
  for (const r of rows) {
    const v = r[col];
    if (v == null) continue;
    if (v instanceof Date) { bail = true; break; }
    const s = String(v).trim();
    if (s === '' || s.length > 80) continue;
    vals.add(s);
    if (vals.size > 30) { bail = true; break; }
  }
