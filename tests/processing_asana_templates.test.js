...
describe('mapAsanaTemplateToProcessing', () => {
  it('maps subtasks→checklist (assignee null) and custom fields→fields', () => {
    ...
    // fields: name + mapped type + gid + default; enum options carried; blank dropped
    expect(m.fields).toEqual([
      ...
      {name: 'Grade', type: 'single', asana_gid: 'cf4', default: 'A', options: ['A', 'B']},
    ]);
...
describe('templateContentKey (idempotency)', () => {
  it('ignores extra field keys + field order-insensitive object keys', () => {
    const a = {
      fields: [{name: 'X', type: 'text', options: ['a'], color: 'red'}],
