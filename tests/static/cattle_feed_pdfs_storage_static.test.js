...
describe('cattle-feed-pdfs append-only storage contract', () => {
  it('all runtime uploads to cattle-feed-pdfs use upsert:false', () => {
    const uploadRe = /\.from\(\s*['"]cattle-feed-pdfs['"]\s*\)\s*\.upload\([\s\S]*?\);/g;
    const offenders = [];
    let uploadCount = 0;
