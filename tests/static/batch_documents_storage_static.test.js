...
describe('batch-documents append-only storage contract', () => {
  it('all runtime uploads to batch-documents use upsert:false', () => {
    const uploadRe = /\.from\(\s*['"]batch-documents['"]\s*\)\s*\.upload\([\s\S]*?\);/g;
    const offenders = [];
    let uploadCount = 0;
