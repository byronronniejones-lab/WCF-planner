...
describe('assertTestDatabase', () => {
  ...
  it('throws when WCF_TEST_DATABASE is missing', () => {
    ...
    expect(() => assertTestDatabase('https://abc123.supabase.co')).toThrow(
      /WCF_TEST_DATABASE/
    );
  });
  ...
  it('throws when URL matches the prod project ref', () => {
    process.env.WCF_TEST_DATABASE = '1';
    expect(() =>
      assertTestDatabase(`https://${_PROD_PROJECT_REF}.supabase.co`)
  ...
  it('throws when URL contains the prod ref anywhere in the string', () => {
    process.env.WCF_TEST_DATABASE = '1';
    expect(() =>
      assertTestDatabase(`https://prefix-${_PROD_PROJECT_REF}-suffix.supabase.co`)
    ).toThrow(/production project ref/);
  });
  ...
  it('passes when flag is "1" and URL is a non-prod project', () => {
    process.env.WCF_TEST_DATABASE = '1';
    expect(() =>
      assertTestDatabase('https://abc123def456.supabase.co')
    ).not.toThrow();
  });
