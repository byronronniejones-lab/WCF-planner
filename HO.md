...
**Latest pushed commit:** `00754c3 feat(weighins): admin broiler session metadata edit`.

For per-commit detail, run `git log --oneline -12` and read PROJECT.md
Part 4 — those are authoritative. Don't duplicate that history here.

**Migrations applied (mig 030–035 authoritative; no new ones this session):**

- 030 `client_submission_id` idempotency columns/indexes (9 tables).
- 031 private `daily-photos` bucket + policies.
- 032 cattle heifer→cow calving trigger.
- 033 cattle calf dam-link trigger.
- 034 `daily_submissions` parent + `submit_add_feed_batch` RPC.
- 035 `submit_weigh_in_session_batch` RPC. Runtime wired Phase 1C-D
...
- PigDailys (standalone webform): flat queue, photos via prepared-photo + atomic IDB enqueue.
- WebformHub broiler/pig/cattle/sheep daily reports: per-form_kind hooks + aggregated stuck modal. Layer + egg stay online-direct.
- WeighIns pig + broiler **fresh draft sessions only**: RPC queue. Cattle/sheep + completion + processor + retag stay online-direct.

**Public broiler WeighIns** reads schooner labels from `webform_config.broiler_batch_meta` only (never `app_store`). Active-only filter via `buildBroilerPublicMirror`.

**Admin broiler `/broiler/weighins`** has an inline metadata-edit panel (WK + team_member); WK change on a complete session recomputes/clears `ppp-v4 wk*Lbs` via `recomputeBroilerBatchWeekAvg(..., {excludeSessionId})`.

**PWA `manifest.webmanifest` `start_url`** is `/webforms`.

**Known working-tree noise** (LF/CRLF only — do not stage casually):
`tests/home_dashboard_equipment.spec.js` + `tests/scenarios/home_dashboard_equipment_seed.js`.

**Watch-outs** (full entries live in PROJECT.md §8 / §7 if applicable):

- Reopen-session broiler avg cleanup is deferred (same staleness class as the WK-edit lane). PROJECT.md §8 Near-term.
- Public broiler grid headers render `'Schooner ' + label` against bare numeric labels — bare-label contract locked by static + Playwright tests.
- PWA install hub is `/webforms`; don't promote separate AddFeed/WeighIns install links.
