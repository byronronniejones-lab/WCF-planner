  ...
  return (
    <div>
      <div style={{display:'flex', alignItems:'center', gap:10, marginBottom:8}}>
        <div style={{fontSize:13, fontWeight:700, color:'#111827', textTransform:'uppercase', letterSpacing:.4}}>Fuel Bills <span style={{color:'#9ca3af', fontWeight:500}}>({bills.length} on file)</span></div>
        <button onClick={()=>setUploadOpen(true)} style={{marginLeft:'auto', padding:'6px 14px', borderRadius:6, border:'none', background:'#085041', color:'white', fontSize:12, fontWeight:600, cursor:'pointer', fontFamily:'inherit'}}>+ Upload bill</button>
      </div>

      {uploadOpen && <BillUploadModal onClose={()=>setUploadOpen(false)} onSaved={()=>{setUploadOpen(false); load();}}/>}

      <div style={{...card, padding:0, overflow:'hidden'}}>
        {bills.length === 0 ? (
          <div style={{padding:24, textAlign:'center', color:'#9ca3af', fontSize:13}}>No bills uploaded yet. Click "+ Upload bill" to parse one.</div>
        ) : (
                ...
                return (
                  <React.Fragment key={b.id}>
                    <tr style={{cursor:'pointer'}} onClick={()=>setExpanded(open ? null : b.id)}>
                      ...
                      <td style={{...td, textAlign:'right', color:'#1e40af', fontWeight:600}}>{Math.round(totalGal).toLocaleString()}</td>
                    ...
                    {open && (
                      <tr><td colSpan={8} style={{padding:0, background:'#fafafa', borderBottom:'1px solid #e5e7eb'}}>
...
function PdfLink({path}) {
  ...
  async function open(e) {
    ...
    if (!url) {
      ...
      if (error) { alert('Cannot open PDF: '+error.message); return; }
  ...
  return <button onClick={open} style={{fontSize:11, color:'#1d4ed8', background:'none', border:'none', textDecoration:'underline', cursor:'pointer', fontFamily:'inherit', padding:0}}>📄 View</button>;
}

function BillDetail({bill, lines, onChanged}) {
  const [busy, setBusy] = React.useState(false);
  async function del() {
    if (!confirm('Delete this bill (and its '+lines.length+' line item'+(lines.length===1?'':'s')+')? PDF will also be removed from storage.')) return;
    setBusy(true);
  ...
  const th = {fontSize:9, fontWeight:700, color:'#6b7280', textTransform:'uppercase', letterSpacing:.4, padding:'5px 10px', textAlign:'left'};
  return (
    <div style={{padding:'14px 18px'}}>
      <div style={{display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(160px, 1fr))', gap:10, fontSize:11, marginBottom:10}}>
        <div><span style={{color:'#9ca3af'}}>Invoice date</span><div style={{fontWeight:600}}>{fmtDate(bill.invoice_date)}</div></div>
        <div><span style={{color:'#9ca3af'}}>Delivery</span><div style={{fontWeight:600}}>{fmtDate(bill.delivery_date)}</div></div>
        <div><span style={{color:'#9ca3af'}}>BOL #</span><div style={{fontWeight:600}}>{bill.bol_number || '—'}</div></div>
        <div><span style={{color:'#9ca3af'}}>Subtotal</span><div style={{fontWeight:600}}>{money(bill.subtotal)}</div></div>
        <div><span style={{color:'#9ca3af'}}>Tax</span><div style={{fontWeight:600}}>{money(bill.tax_total)}</div></div>
        <div><span style={{color:'#9ca3af'}}>Total</span><div style={{fontWeight:600, color:'#065f46'}}>{money(bill.total)}</div></div>
      </div>
        ...
        <tbody>
          {lines.map(l => (
            <tr key={l.id}>
              ...
              <td style={{...td, textAlign:'right'}}>{l.gross_units != null ? Number(l.gross_units).toLocaleString() : '—'}</td>
              <td style={{...td, textAlign:'right', fontWeight:600}}>{l.net_units != null ? Number(l.net_units).toLocaleString() : '—'}</td>
              <td style={{...td, textAlign:'right'}}>{l.unit_price != null ? '$'+Number(l.unit_price).toFixed(4) : '—'}</td>
              ...
              <td style={{...td, textAlign:'right', color:'#065f46', fontWeight:700}}>{l.effective_per_gal != null ? '$'+Number(l.effective_per_gal).toFixed(4) : '—'}</td>
            </tr>
          ))}
        </tbody>
      </table>
      <div style={{marginTop:10, display:'flex'}}>
        <button onClick={del} disabled={busy} style={{marginLeft:'auto', padding:'5px 12px', borderRadius:5, border:'1px solid #fecaca', background:'white', color:'#b91c1c', fontSize:11, fontWeight:600, cursor:'pointer', fontFamily:'inherit'}}>Delete bill</button>
      </div>
    </div>
  );
}
...
function BillUploadModal({onClose, onSaved}) {
  ...
  const inp = {fontSize:13, padding:'6px 9px', border:'1px solid #d1d5db', borderRadius:5, fontFamily:'inherit', width:'100%', boxSizing:'border-box'};
  const lbl = {fontSize:10, color:'#6b7280', fontWeight:600, textTransform:'uppercase', letterSpacing:.4, marginBottom:3, display:'block'};
  const td = {fontSize:11, padding:'5px 8px'};

  return (
    <div onClick={e=>{if(e.target===e.currentTarget) onClose();}} style={{position:'fixed', inset:0, background:'rgba(17,24,39,.6)', display:'flex', alignItems:'flex-start', justifyContent:'center', padding:'24px 16px', overflowY:'auto', zIndex:1000}}>
      <div style={{background:'white', borderRadius:14, width:'100%', maxWidth:980, boxShadow:'0 20px 40px rgba(0,0,0,.3)', overflow:'hidden'}}>
        <div style={{padding:'14px 20px', borderBottom:'1px solid #e5e7eb', display:'flex', alignItems:'center', gap:12}}>
          <div style={{flex:1}}>
            <div style={{fontSize:10, fontWeight:700, color:'#6b7280', textTransform:'uppercase', letterSpacing:.5}}>Upload</div>
            <div style={{fontSize:16, fontWeight:700, color:'#111827'}}>Fuel Bill (PDF)</div>
          </div>
          <button onClick={onClose} style={{fontSize:14, padding:'6px 14px', borderRadius:6, border:'1px solid #d1d5db', background:'white', color:'#374151', cursor:'pointer', fontFamily:'inherit', fontWeight:600}}>✕ Close</button>
        </div>

        <div style={{padding:'16px 20px'}}>
          <div style={{padding:14, border:'1px dashed #d1d5db', borderRadius:8, background:'#fafafa', marginBottom:14}}>
            <input type="file" accept="application/pdf" onChange={e=>pickAndParse(e.target.files && e.target.files[0])}/>
            {file && <span style={{marginLeft:10, fontSize:12, color:'#6b7280'}}>{file.name} · {Math.round(file.size/1024)} KB</span>}
          ...
          {parsed && parsed.warnings.length > 0 && (
            <div style={{padding:'10px 14px', background:'#fffbeb', border:'1px solid #fde68a', borderRadius:8, marginBottom:14, fontSize:12, color:'#92400e'}}>
              ...
              <ul style={{margin:0, paddingLeft:20}}>{parsed.warnings.map((w, i) => <li key={i}>{w}</li>)}</ul>
            </div>
          )}

          {parsed && (
            <>
              <div style={{fontSize:11, fontWeight:700, color:'#4b5563', textTransform:'uppercase', letterSpacing:.5, marginBottom:8}}>Header</div>
              <div style={{display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(150px, 1fr))', gap:10, marginBottom:14}}>
                <div><label style={lbl}>Supplier</label><input style={inp} value={parsed.header.supplier || ''} onChange={e=>setHeader('supplier', e.target.value)}/></div>
                <div><label style={lbl}>Invoice #</label><input style={inp} value={parsed.header.invoice_number || ''} onChange={e=>setHeader('invoice_number', e.target.value)}/></div>
                <div><label style={lbl}>Invoice date</label><input type="date" style={inp} value={parsed.header.invoice_date || ''} onChange={e=>setHeader('invoice_date', e.target.value)}/></div>
                <div><label style={lbl}>Delivery date *</label><input type="date" style={inp} value={parsed.header.delivery_date || ''} onChange={e=>setHeader('delivery_date', e.target.value)}/></div>
                <div><label style={lbl}>BOL #</label><input style={inp} value={parsed.header.bol_number || ''} onChange={e=>setHeader('bol_number', e.target.value)}/></div>
                <div><label style={lbl}>Subtotal</label><input type="number" step="0.01" style={inp} value={parsed.header.subtotal || ''} onChange={e=>setHeader('subtotal', e.target.value)}/></div>
                <div><label style={lbl}>Tax</label><input type="number" step="0.01" style={inp} value={parsed.header.tax_total || ''} onChange={e=>setHeader('tax_total', e.target.value)}/></div>
                <div><label style={lbl}>Total *</label><input type="number" step="0.01" style={inp} value={parsed.header.total || ''} onChange={e=>setHeader('total', e.target.value)}/></div>
              </div>

              <div style={{fontSize:11, fontWeight:700, color:'#4b5563', textTransform:'uppercase', letterSpacing:.5, marginBottom:8}}>Line items <span style={{color:'#9ca3af', fontWeight:400}}>({parsed.lines.length})</span></div>
                  ...
                  <tbody>
                    {parsed.lines.map((l, i) => (
                      <tr key={i} style={{borderTop:'1px solid #f3f4f6'}}>
                        <td style={td}><input style={{...inp, fontSize:11}} value={l.description || ''} onChange={e=>setLine(i, 'description', e.target.value)}/></td>
                        <td style={td}>
                          <select style={{...inp, fontSize:11}} value={l.fuel_type || ''} onChange={e=>setLine(i, 'fuel_type', e.target.value)}>
              ...
              {err && <div style={{padding:'8px 12px', background:'#fef2f2', border:'1px solid #fecaca', color:'#b91c1c', borderRadius:5, fontSize:12, marginBottom:10}}>{err}</div>}

              <div style={{display:'flex', gap:8, justifyContent:'flex-end'}}>
                <button onClick={onClose} style={{padding:'8px 16px', borderRadius:6, border:'1px solid #d1d5db', background:'white', color:'#374151', fontSize:13, fontWeight:600, cursor:'pointer', fontFamily:'inherit'}}>Cancel</button>
                <button onClick={save} disabled={busy} style={{padding:'8px 18px', borderRadius:6, border:'none', background:busy?'#9ca3af':'#085041', color:'white', fontSize:13, fontWeight:700, cursor:busy?'not-allowed':'pointer', fontFamily:'inherit'}}>{busy?'Saving…':'Save bill'}</button>
              </div>
            </>
          )}

          {!parsed && !parsing && (
            <div style={{padding:30, textAlign:'center', color:'#9ca3af', fontSize:13}}>Pick a Home Oil PDF to parse. Other suppliers may need manual edits after parsing.</div>
          )}
        </div>
      </div>
    </div>
