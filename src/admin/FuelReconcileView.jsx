...
export default function FuelReconcileView() {
  ...
  async function load() {
    ...
    const [billsRes, linesRes, supRes, fuelRes] = await Promise.all([
      ...
      sb.from('equipment_fuelings').select('id,date,gallons,def_gallons,fuel_type,suppressed').order('date',{ascending:false}).limit(20000),
    ]);
    ...
    <div>
      <div style={{...card, background:'#f0fdf4', borderColor:'#86efac'}}>
        <div style={{fontSize:12, color:'#065f46', lineHeight:1.5}}>
          <strong>How this works:</strong> <strong>Purchased</strong> = supplier bills (Home Oil etc.) grouped by delivery month. <strong>Consumed</strong> = per-piece equipment fueling checklists at <code>/fueling/&lt;piece&gt;</code> plus direct non-equipment dispensing at <code>/fueling/supply</code> (gas cans, farm truck, other). Cell refills (fuel going INTO the portable cell from a supplier delivery) are inventory movement, NOT consumption — they're excluded so we don't double-count once the equipment checklists record what was later pulled out of the cell. Variance bands: <strong>&le;{VARIANCE_WARN_PCT}% green</strong> · <strong>{VARIANCE_WARN_PCT}–{VARIANCE_WARN_PCT*2}% orange</strong> · <strong>&gt;{VARIANCE_WARN_PCT*2}% red</strong>. <em>Single-month variance is rarely meaningful</em> — month timing (bill arrives one month, fuel gets consumed the next) and inventory carryover in the cell make exact monthly matches uncommon. Look at multi-month trends.
        </div>
      </div>

      <div style={{...card, padding:0, overflow:'auto'}}>
        {sorted.length === 0 ? (
          <div style={{padding:30, textAlign:'center', color:'#9ca3af', fontSize:13}}>No bills, equipment fuelings, or direct fuel logs yet.</div>
        ) : (
          <table style={{width:'100%', borderCollapse:'collapse', minWidth:1100}}>
            <thead>
              <tr>
                <th rowSpan={2} style={th}>Month</th>
                <th colSpan={4} style={{...th, borderBottom:'1px solid #d1d5db', textAlign:'center'}}>Purchased (bills)</th>
                <th colSpan={3} style={{...th, borderBottom:'1px solid #d1d5db', textAlign:'center'}}>Consumed (equipment + direct)</th>
