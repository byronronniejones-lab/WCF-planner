...
const SheepNewWeighInModal = ({sb, onClose, onCreate}) => {
  ...
  const FLOCK_OPTS = [{v:'rams',l:'Rams'},{v:'ewes',l:'Ewes'},{v:'feeders',l:'Feeders'}];
  useEffect(() => {
    sb.from('webform_config').select('data').eq('key','per_form_team_members').maybeSingle().then(({data}) => {
      const perForm = data && data.data || {};
      if(Array.isArray(perForm['sheep-weighins']) && perForm['sheep-weighins'].length > 0) { setTeamMembers(perForm['sheep-weighins']); return; }
  ...
  async function create() {
    ...
    if(!herd) { setErr('Pick a flock.');       return; }
    setErr(''); setBusy(true);
  ...
  return (
    <div style={{position:'fixed', inset:0, background:'rgba(0,0,0,.45)', zIndex:500, display:'flex', alignItems:'center', justifyContent:'center', padding:16}}>
      <div style={{background:'white', borderRadius:12, maxWidth:420, width:'100%', padding:'18px 20px', boxShadow:'0 12px 40px rgba(0,0,0,.25)'}}>
        ...
        <div style={{marginBottom:14}}><label style={lblS}>Flock *</label>
          <select value={herd} onChange={e=>setHerd(e.target.value)} style={inpS}>
            <option value=''>Select flock...</option>
            {FLOCK_OPTS.map(h => <option key={h.v} value={h.v}>{h.l}</option>)}
          </select>
        </div>
        ...
        <div style={{display:'flex', gap:8, justifyContent:'flex-end'}}>
          ...
          <button onClick={create} disabled={busy || !team || !herd} style={{padding:'8px 16px', borderRadius:7, border:'none', background:(busy||!team||!herd)?'#9ca3af':'#0f766e', color:'white', fontWeight:700, fontSize:12, cursor:(busy||!team||!herd)?'not-allowed':'pointer', fontFamily:'inherit'}}>{busy?'Creating…':'Create Session'}</button>
        </div>
      </div>
    </div>
  );
