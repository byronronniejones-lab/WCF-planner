...
export default function SheepSendToProcessorModal({sb, session, flaggedEntries, sheepList, teamMember, onCancel, onConfirmed}) {
  ...
  React.useEffect(() => {
    ...
    sb.from('sheep_processing_batches')
      ...
      .then(({data, error}) => {
        ...
        const existingNums = rows.map(b => (b.name||'').startsWith('S-'+yr+'-') ? parseInt(b.name.slice(5))||0 : 0);
        ...
        {!loading && (
          <div style={{marginBottom:10}}>
            <div style={{display:'flex', gap:6, marginBottom:8}}>
              <button type="button" onClick={()=>setMode('existing')} disabled={plannedBatches.length===0} style={{flex:1, padding:'7px 10px', borderRadius:6, border:'1px solid '+(mode==='existing'?'#0f766e':'#d1d5db'), background:mode==='existing'?'#0f766e':'white', color:mode==='existing'?'white':(plannedBatches.length===0?'#d1d5db':'#374151'), fontSize:12, fontWeight:600, cursor:plannedBatches.length===0?'not-allowed':'pointer', fontFamily:'inherit'}}>{'Existing planned ('+plannedBatches.length+')'}</button>
              <button type="button" onClick={()=>setMode('new')} style={{flex:1, padding:'7px 10px', borderRadius:6, border:'1px solid '+(mode==='new'?'#0f766e':'#d1d5db'), background:mode==='new'?'#0f766e':'white', color:mode==='new'?'white':'#374151', fontSize:12, fontWeight:600, cursor:'pointer', fontFamily:'inherit'}}>+ New batch</button>
            </div>
            ...
            {mode === 'existing' && (
              <select value={batchId} onChange={e=>setBatchId(e.target.value)} style={inpS}>
                ...
                {plannedBatches.map(b => {
                  ...
                  return <option key={b.id} value={b.id}>{b.name+when+' · '+n+(n===1?' sheep':' sheep')}</option>;
                })}
              </select>
            )}
            ...
            {mode === 'new' && (
              <div style={{display:'flex', gap:8}}>
                <div style={{flex:1}}>
                  ...
                  <input type="text" value={newName} onChange={e=>setNewName(e.target.value)} placeholder="S-26-01" style={inpS}/>
                </div>
                <div style={{width:140}}>
