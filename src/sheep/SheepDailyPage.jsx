...
export default function SheepDailyPage({sb, fmt, authState, Header}) {
  ...
  async function loadAll() {
    const {data} = await sb.from('sheep_dailys').select('*').eq('id', recordId).is('deleted_at', null).single();
  ...
  React.useEffect(() => {
    ...
    (async () => {
      try {
        const rows = await listActivityEvents(sb, 'sheep.daily', record.id, {limit: 50});
        if (cancelled) return;
  ...
  React.useEffect(() => {
    if (!record) return;
    listActivityEvents(sb, 'sheep.daily', record.id, {limit: 50}).then((rows) => {
      const auditOnly = (rows || []).filter((e) => e.event_type !== 'comment.posted');
      setActivityCount(auditOnly.filter((e) => !e.deleted_at).length);
    });
  }, [record?.id]);
      ...
      <div style={{maxWidth: 800, margin: '0 auto', padding: '12px 16px'}}>
        ...
        >
          ...
          <div style={fieldRow}>
            ...
            <span>{record.fence_voltage_kv != null ? record.fence_voltage_kv + ' kV' : '—'}</span>
          </div>
          <div style={fieldRow}>
            <span style={fieldLabel}>Waterers working</span>
            <span>{record.waterers_working === false ? 'No' : 'Yes'}</span>
          </div>
          <div style={fieldRow}>
