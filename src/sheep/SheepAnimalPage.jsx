...
export default function SheepAnimalPage({sb, fmt, authState, Header}) {
  ...
  React.useEffect(() => {
    if (!activityExpanded || !animal) return;
    let cancelled = false;
    (async () => {
      try {
        const rows = await listActivityEvents(sb, 'sheep.animal', animal.id, {limit: 50});
        if (cancelled) return;
  ...
  }, [activityExpanded, animal?.id]);

  React.useEffect(() => {
    if (!animal) return;
    listActivityEvents(sb, 'sheep.animal', animal.id, {limit: 50}).then((rows) => {
      const auditOnly = (rows || []).filter((e) => e.event_type !== 'comment.posted');
      setActivityCount(auditOnly.filter((e) => !e.deleted_at).length);
    });
  }, [animal?.id]);
  ...
  React.useEffect(() => {
    function onActivityChange() {
      if (!animal || !activityExpanded) return;
      listActivityEvents(sb, 'sheep.animal', animal.id, {limit: 50}).then((rows) => {
        const auditOnly = (rows || []).filter((e) => e.event_type !== 'comment.posted');
        setActivityEvents(auditOnly);
        setActivityCount(auditOnly.filter((e) => !e.deleted_at).length);
