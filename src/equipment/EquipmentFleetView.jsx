...
export default function EquipmentFleetView({sb, equipment, fuelings, fmt, onOpen, onReload}) {
  ...
  for (const f of (fuelings || [])) {
    ...
    (f.service_intervals_completed || []).forEach(c => arr.push({
      ...c,
  ...
  const tile = (eq) => {
    ...
    const warrantyExpiresSoon = eq.warranty_expiration && daysSince(eq.warranty_expiration) != null && daysSince(eq.warranty_expiration) > -WARRANTY_WINDOW_DAYS && daysSince(eq.warranty_expiration) < 0;
    ...
    return (
      <div key={eq.id} onClick={()=>onOpen(eq.slug)} className="hoverable-tile"
        style={{background:'white', border:'1px solid '+cat.bd, borderRadius:12, padding:'14px 18px', cursor:'pointer', transition:'transform .1s', minWidth:0}}>
        <div style={{display:'flex', alignItems:'center', gap:10, marginBottom:10, flexWrap:'wrap'}}>
          <span style={{fontSize:15, fontWeight:700, color:cat.color}}>{eq.name}</span>
          <span style={{fontSize:10, fontWeight:700, padding:'2px 8px', borderRadius:10, background:eq.status==='active'?'#d1fae5':'#f3f4f6', color:eq.status==='active'?'#065f46':'#374151', textTransform:'uppercase'}}>{eq.status}</span>
        ...
        <div style={{display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(120px, 1fr))', gap:10, fontSize:11}}>
          <div>
            <div style={{color:'#9ca3af', fontSize:10, textTransform:'uppercase', letterSpacing:.4}}>{eq.tracking_unit === 'km' ? 'Current KM' : 'Current Hours'}</div>
            <div style={{fontWeight:700, color:'#111827'}}>{fmtReading(reading, eq.tracking_unit)}</div>
          </div>
          {latestFuel && (
            <div>
              <div style={{color:'#9ca3af', fontSize:10, textTransform:'uppercase', letterSpacing:.4}}>Last fueling</div>
              <div style={{fontWeight:600, color:missedFuel?'#b91c1c':'#111827'}}>{fmt(latestFuel.date)}{daysSinceFuel != null ? ' ('+daysSinceFuel+'d)' : ''}</div>
            </div>
          )}
          {dueInfo && (
            <div>
              <div style={{color:'#9ca3af', fontSize:10, textTransform:'uppercase', letterSpacing:.4}}>Next service</div>
              <div style={{fontWeight:700, color:dueInfo.overdue?'#b91c1c':'#a16207'}}>
                {dueInfo.overdue ? ('OVERDUE '+dueInfo.hours_or_km+dueInfo.kind.charAt(0)) : (dueInfo.hours_or_km+dueInfo.kind.charAt(0)+' at '+dueInfo.next_due)}
              </div>
            </div>
          )}
          {warrantyExpiresSoon && (
            <div>
              <div style={{color:'#9ca3af', fontSize:10, textTransform:'uppercase', letterSpacing:.4}}>Warranty</div>
      ...
      {grouped.map(g => (
        <div key={g.key}>
          <div style={{display:'flex', alignItems:'center', gap:10, marginBottom:8}}>
            ...
            <span style={{fontSize:14, fontWeight:700, color:g.color, textTransform:'uppercase', letterSpacing:.4}}>{g.label}</span>
            <span style={{fontSize:11, color:'#6b7280'}}>{g.rows.length} piece{g.rows.length===1?'':'s'}</span>
          </div>
