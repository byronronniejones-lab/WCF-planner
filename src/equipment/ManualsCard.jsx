    ...
    <div style={{background:'white', border:'1px solid #e5e7eb', borderRadius:12, marginBottom:12, overflow:'hidden'}}>
      <button
        ...
        <span style={{flex:1, fontSize:13, fontWeight:700, color:'#4b5563'}}>
          Manuals &amp; Videos <span style={{color:'#9ca3af', fontWeight:500, marginLeft:6}}>({manuals.length})</span>
        </span>
        ...
        <div style={{padding:'0 16px 14px'}}>
          {pdfs.length > 0 && (
            <div style={{marginBottom: videos.length > 0 ? 12 : 0}}>
              <div style={{fontSize:10, fontWeight:700, color:'#92400e', textTransform:'uppercase', letterSpacing:.4, marginBottom:6}}>Manuals &amp; PDFs</div>
              <div style={{display:'flex', flexDirection:'column', gap:5}}>
                {pdfs.map((m, i) => (
                  <a key={i} href={m.url} target="_blank" rel="noopener noreferrer"
                    style={{display:'flex', alignItems:'center', gap:10, padding:'8px 10px', background:'#fffbeb', border:'1px solid #fde68a', borderRadius:6, textDecoration:'none', color:'#92400e', fontSize:13}}>
                    <span style={{fontSize:16}}>📄</span>
          ...
          {videos.length > 0 && (
            <div>
              <div style={{fontSize:10, fontWeight:700, color:'#991b1b', textTransform:'uppercase', letterSpacing:.4, marginBottom:6}}>Videos</div>
              <div style={{display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(200px, 1fr))', gap:8}}>
                {videos.map((m, i) => {
                  ...
                  return (
                    <a key={i} href={m.url} target="_blank" rel="noopener noreferrer"
                      style={{display:'block', background:'#fef2f2', border:'1px solid #fecaca', borderRadius:6, overflow:'hidden', textDecoration:'none', color:'#991b1b'}}>
                      {thumb ? (
                        <div style={{position:'relative', width:'100%', paddingBottom:'56.25%', background:'#111'}}>
                          ...
                          <div style={{position:'absolute', top:'50%', left:'50%', transform:'translate(-50%,-50%)', width:46, height:46, borderRadius:23, background:'rgba(0,0,0,.65)', color:'white', fontSize:22, display:'flex', alignItems:'center', justifyContent:'center', paddingLeft:3}}>▶</div>
                        </div>
                      ) : (
                        <div style={{padding:'18px 12px', textAlign:'center', background:'#fafafa'}}>▶</div>
                      )}
                      <div style={{padding:'8px 10px', fontSize:12, fontWeight:600}}>{m.title || 'YouTube video'}</div>
                    </a>
                  );
                })}
              </div>
